...
export function scannableRepos(): Repo[] {
  return Object.values(REPOS).filter((r) => !r.sharedCheckout);
}
...
7. For a confirmed finding, branch off origin/${repo.defaultBranch}: \`git checkout -b deepsec-scan-<short-slug> origin/${repo.defaultBranch}\`. Write a MINIMAL fix targeting the root cause only.${isFusion ? `
   - Webapp ReScript (.res) change → run \`bun rescript-check\` in packages/core/webapp and resolve errors before committing. Treat .res as source of truth; never edit generated .bs.js.` : ""}
...
8. Commit, push, then \`gh pr create --repo ${repo.ghRepo} --base ${repo.defaultBranch} --title "[deepsec][scan] <concise title>" --body "<finding, impact, fix, severity; note: generated by an on-demand Backstage security scan via vercel-labs/deepsec>"\`. One PR per finding. NEVER run \`gh pr merge\`.
