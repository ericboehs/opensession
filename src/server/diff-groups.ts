...
const cache: Map<string, CacheEntry> = ((globalThis as any).__diffGroupsCache ??=
	new Map());
const inflight: Map<string, Promise<DiffFileGroup[] | null>> = ((globalThis as any)
...
export function diffGroupsFingerprint(
	...
	return createHash("sha256")
		...
		.update(
			JSON.stringify(
				files.map(({ path, oldPath, status, additions, deletions, binary }) => ({
					path,
					oldPath,
					status,
					additions,
...
export function normalizeDiffGroups(
	...
	try {
		...
		for (const candidate of parsed.groups) {
			...
			if (!title || title.length > 40 || !Array.isArray(candidate?.files)) continue;
			const paths: string[] = [];
			for (const path of candidate.files) {
				if (typeof path !== "string" || !expected.has(path) || assigned.has(path)) continue;
				assigned.add(path);
				paths.push(path);
			}
...
export async function getDiffFileGroups(
	...
	const promise = generate(repo, files, patch).finally(() => inflight.delete(key));
