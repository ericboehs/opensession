...
End with a short digest: PRs ready to merge, PRs needing attention (one line each on why), and anything idle >7 days. Keep it terse — this is a morning briefing, not a full review.`,
  },
  {
    ...
    description: "Daily look at Sentry + Grafana for new or spiking production errors, with a triage note per issue.",
...
Output a ranked list with your triage notes. Do not attempt fixes in this run — recommend which issues deserve a dedicated session.`,
  },
  {
    ...
    description: "Weekly rollup of recurring themes in support tickets, with product suggestions.",
    category: "digest",
    schedule: "0 8 * * 1",
    mode: "ask",
...
Write the rollup in English regardless of ticket language. Rank themes by ticket volume. End with the top 3 things most worth fixing this week.`,
  },
  {
    ...
    description: "Every Friday, draft a user-facing changelog from the week's merged PRs and completed Linear issues.",
    category: "digest",
    schedule: "0 14 * * 5",
    mode: "ask",
    mcpServers: ["linear"],
...
Write it as a short marketing-friendly changelog: features first, then improvements, then fixes. One line each, written for ${personaProduct()} users, not engineers. Flag anything you're unsure is user-visible.`,
  },
  {
    id: "stale-pr-nudge",
    name: "Stale PR nudge",
    description: "Daily check for PRs idle >3 days; posts one nudge listing them and who they're waiting on.",
    category: "hygiene",
    schedule: "0 9 * * 1-5",
    mode: "ask",
    mcpServers: [],
...
Summarize as a short list ordered by staleness. If nothing is stale, say so in one line.`,
  },
  {
    ...
    description: "Weekly check for outdated/vulnerable dependencies; opens a PR bumping the safe ones.",
    category: "hygiene",
    schedule: "0 6 * * 2",
    mode: "code",
    mcpServers: [],
...
If everything is current, don't open a PR — just report that.`,
  },
  {
    ...
    description: "Weekly hunt through recent CI runs for flaky tests; opens a PR fixing or quarantining the worst.",
    category: "hygiene",
    schedule: "0 6 * * 3",
    mode: "code",
    mcpServers: [],
...
If you find no flakes, report that with the runs you checked.`,
  },
  {
    ...
    description: "On every new support ticket: investigate the customer, find the cause, leave an internal note with a suggested reply.",
    category: "triage",
    schedule: "",
...
Never reply to the customer directly and never change the thread state.`,
  },
  {
    ...
    description: "On every new chargeback: gather evidence about the customer and their usage, drop a dossier note.",
    category: "investigator",
    schedule: "",
...
Assess: does the usage pattern support "legitimate customer who used the product" (good evidence to fight the dispute) or does it look like fraud/abuse? Recommend fight vs. accept, and list the specific evidence to submit if fighting. Do NOT take any Stripe action yourself — a human submits the response.`,
  },
  {
    ...
    description: "Each morning, summarize what happened across the team's Slack channels while you were away.",
    category: "digest",
    schedule: "0 6 * * 1-5",
    mode: "ask",
    mcpServers: ["slack"],
