...
export function codeFlowHttpError(error: unknown): { message: string; status: number } {
	...
	if (message === "The remote workspace is unavailable") return { message, status: 503 };
...
function enterRequest() {
	if (activeRequests >= 2) throw new Error("Code-flow analysis is busy; try again");
	activeRequests++;
}

function validPath(path: string): boolean {
	return Boolean(path) && !path.includes("\0") && !path.startsWith("/") &&
		!path.split(/[\\/]/).includes("..");
}
...
function rememberSnapshot(key: string, result: CodeFlowResult) {
	...
	while (snapshotCache.size > MAX_CACHE) snapshotCache.delete(snapshotCache.keys().next().value!);
}

function fingerprint(repo: string, base: string, head: string, diffVersion: string, pairs: CodeFlowSourcePair[], skipped: number): string {
	const hash = createHash("sha256")
		.update(repo).update("\0")
	...
	for (const pair of pairs) {
		for (const value of [pair.oldPath, pair.path, pair.before, pair.after]) {
			...
			const text = value ?? "";
			hash.update(tag).update(":").update(String(Buffer.byteLength(text))).update(":").update(text);
		}
	}
...
function analyzeInWorker(input: CodeFlowAnalysisInput): Promise<CodeFlowResult> {
	if (pendingWorkers >= 2) return Promise.reject(new Error("Code-flow analysis is busy; try again"));
	pendingWorkers++;
	const run = workerQueue.catch(() => {}).then(() => new Promise<CodeFlowResult>((resolveResult, reject) => {
		...
		worker.onmessage = (event: MessageEvent<{ ok: boolean; result?: CodeFlowResult; error?: string }>) => {
			if (!done()) return;
			if (event.data.ok && event.data.result) resolveResult(event.data.result);
			else reject(new Error(event.data.error || "Code-flow analysis failed"));
		};
		worker.onerror = () => {
			if (!done(true)) return;
...
async function cachedAnalysis(repo: string, base: string, head: string, diffVersion: string, pairs: CodeFlowSourcePair[], skipped: number, cacheable = true) {
	const key = fingerprint(repo, base, head, diffVersion, pairs, skipped);
	if (!cacheable)
		return analyzeInWorker({ repo, base, head, diffVersion, pairs, skippedFiles: skipped });
	const hit = cache.get(key);
	if (hit) return hit;
	const running = inflight.get(key);
	if (running) return running;
	const promise = analyzeInWorker({ repo, base, head, diffVersion, pairs, skippedFiles: skipped });
...
async function execText(exec: WorkspaceExec, args: string[]): Promise<string | null> {
	const result = await exec(args);
	return result.exitCode === 0 ? result.stdout : null;
}

async function gitFile(exec: WorkspaceExec, ref: string, path: string, limit: number): Promise<string | null> {
	if (!validPath(path)) return null;
	const entry = await execText(exec, ["git", "ls-tree", ref, "--", `:(literal)${path}`]);
	if (!entry || !/^100(?:644|755) blob /.test(entry)) return null;
	const sizeText = await execText(exec, ["git", "cat-file", "-s", `${ref}:${path}`]);
...
async function worktreeFile(exec: WorkspaceExec, root: string, path: string, limit: number, canonicalRoot?: string): Promise<string | null> {
	if (!validPath(path)) return null;
	if (exec.remote) {
		return execText(exec, ["/home/ubuntu/.bun/bin/bun", "-e", REMOTE_BOUNDED_READ, path, String(limit)]);
	}
	const full = resolve(root, path);
	const resolvedRoot = canonicalRoot ?? await realpath(root).catch(() => resolve(root));
	const prefix = resolvedRoot.endsWith(sep) ? resolvedRoot : `${resolvedRoot}${sep}`;
	const preCanonical = await realpath(full).catch(() => "");
	if (!preCanonical.startsWith(prefix)) return null;
	const handle = await open(preCanonical, constants.O_RDONLY | constants.O_NOFOLLOW).catch(() => null);
	...
	try {
		...
		const current = canonicalFile ? await fileStat(canonicalFile).catch(() => null) : null;
		if (!stat.isFile() || stat.size > limit || !current || current.dev !== stat.dev || current.ino !== stat.ino || !canonicalFile.startsWith(prefix)) return null;
		const buffer = Buffer.alloc(limit + 1);
		let offset = 0;
		while (offset < buffer.length) {
			const { bytesRead } = await handle.read(buffer, offset, buffer.length - offset, null);
...
function eligible(files: Array<Pick<DiffFile, "path" | "oldPath" | "status">>) {
	return files.filter((file) =>
		...
		(supportsCodeFlow(file.path) || Boolean(file.oldPath && supportsCodeFlow(file.oldPath))),
	);
}
...
async function readPairs(
	...
	for (const file of candidates.slice(0, MAX_FILES)) {
		...
		const before = needsBefore
			? await readBefore(file.oldPath ?? file.path, Math.min(MAX_FILE_BYTES, remaining))
			: null;
		...
		if ((!before && !after) || before?.includes("\0") || after?.includes("\0")) {
			skipped++;
			continue;
		}
		total += bytes;
...
export async function sessionCodeFlow(session: UnifiedSession, repoId?: string): Promise<CodeFlowResult> {
	enterRequest();
	try {
		...
		throw new Error("The workspace changed during code-flow analysis; try again");
	} finally {
		activeRequests--;
	}
}
...
async function git(dir: string, args: string[]): Promise<string> {
	...
	if (timedOut || code !== 0)
		throw new Error(timedOut ? "Git snapshot timed out" : "Git snapshot unavailable");
	return stdout.trim();
}
...
async function ensurePrObjects(repo: Repo, details: PrDetails, diff: PrDiffData) {
	...
	try {
		...
		const headSpec = repo.host === "codestorage"
		...
		if (fetchedHead !== diff.headRefOid) throw new Error("Pull request changed during code-flow analysis");
		if (diff.baseRefOid) {
...
function boundedPatch(patch: string): { patch: string; skipped: number } {
	...
	return { patch: boundary > 0 ? prefix.slice(0, boundary + 1) : "", skipped: 1 };
}

export async function prCodeFlow(repo: Repo, details: PrDetails, diff: PrDiffData): Promise<CodeFlowResult> {
	enterRequest();
	let refs: Awaited<ReturnType<typeof ensurePrObjects>> | null = null;
	try {
		const version = diff.diffVersion ?? createHash("sha256").update(diff.patch).digest("base64url");
