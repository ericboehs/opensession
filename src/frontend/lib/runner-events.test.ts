...
describe("isTimelineOnlyRunnerNotice", () => {
	...
	test("keeps ordinary assistant and diagnostic text", () => {
		expect(isTimelineOnlyRunnerNotice("Falling back to the simpler implementation.")).toBe(
			false,
		);
		expect(isTimelineOnlyRunnerNotice("[runner] account rotation complete")).toBe(false);
	});
});
