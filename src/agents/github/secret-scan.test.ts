...
import { $ } from "bun";
import { parseAddedLines, runSecretScanCheck, secretScanSection } from "./secret-scan";
...
describe("parseAddedLines", () => {
  test("maps -U0 hunks to added new-file line numbers per file", () => {
    ...
    expect([...(m.get("src/a.ts") || [])].sort((x, y) => x - y)).toEqual([11, 12, 23]);
    expect(m.has("gone.ts")).toBe(false);
    expect([...(m.get("new.ts") || [])].sort((x, y) => x - y)).toEqual([1, 2, 3]);
  });
...
async function makeRepo(dir: string, headEdit: (dir: string) => void): Promise<void> {
...
describe("runSecretScanCheck", () => {
  test("reports a finding on an added line and maps the path back", async () => {
    ...
    try {
      await makeRepo(dir, (d) => writeFileSync(join(d, "src/creds.ts"), "l1\nl2\nSECRET\nl4\nl5\n"));
      ...
      expect(res.findings).toEqual([
        { file: "src/creds.ts", line: 3, detector: "FakeDetector", redacted: "AKIA****", verified: true },
      ]);
  ...
  test("skips cleanly when the PR only deletes lines", async () => {
    ...
    try {
      ...
      const res = await runSecretScanCheck({ cwd: dir, baseRefName: "main", prNumber: 1, bin: makeFakeScanner(dir, 1) });
      expect(res.findings).toEqual([]);
    } finally {
      rmSync(dir, { recursive: true, force: true });
    }
  ...
  test.if(existsSync(realBin))("real trufflehog run completes and stays quiet on a fake key", async () => {
    const dir = mkdtempSync(join(tmpdir(), "ss-test-"));
    try {
      await makeRepo(dir, (d) =>
        writeFileSync(join(d, "src/creds.ts"), "l1\nl2\nSTRIPE=sk_live_51AbCdEfGhIjKlMnOpQrStUvWxYz01234567\nl4\nl5\n"),
      );
      const res = await runSecretScanCheck({ cwd: dir, baseRefName: "main", prNumber: 1, bin: realBin });
      expect(res.skipped).toBe("");
