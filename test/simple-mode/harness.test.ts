...
describe("simple mode install", () => {
  ...
  test("turn: one real agent turn (needs OPENSESSION_TEST_CLAUDE_TOKEN)", async () => {
    ...
    const status = await guest(`curl -fsS http://127.0.0.1:${PORT}/api/setup/status`);
    expectOk(status, "engine status");
    const st =
      (JSON.parse(status.stdout) as { engine?: { ready?: boolean; claudeAccounts?: number; blocker?: string } }).engine ?? {};
