...
if (command === "start") {
	const owner = (flag("owner", "unknown") || "unknown").replace(/[^a-zA-Z0-9_-]/g, "").slice(0, 24);
...
if (command === "worker") {
	...
	if (!port || !display || !profile || !state) throw new Error("worker requires port, display, profile, and state");
	...
	if (await Bun.file(socket).exists()) throw new Error(`X display :${display} is already in use`);
	mkdirSync(profile, { recursive: true });
	const xvfb = Bun.spawn(["/usr/bin/Xvfb", `:${display}`, "-screen", "0", "1600x1100x24"], {
	...
	const chrome = Bun.spawn([
		...
		"--window-size=1600,1100", `--user-data-dir=${profile}`, "about:blank",
	], { env: { ...process.env, DISPLAY: `:${display}` }, stdin: "ignore", stdout: "ignore", stderr: "ignore" });
	let ready = false;
	for (let i = 0; i < 100; i++) {
		ready = await fetch(`http://127.0.0.1:${port}/json/version`).then((r) => r.ok).catch(() => false);
		if (ready) break;
		if (chrome.exitCode !== null) break;
		await sleep(100);
	}
	if (!ready) {
		chrome.kill(); xvfb.kill(); rmSync(profile, { recursive: true, force: true });
		throw new Error("Chrome did not expose CDP within 10 seconds");
	}
	writeFileSync(state, JSON.stringify({ port, display, profile }));
	let idleSince = Date.now();
	const stop = () => { chrome.kill(); xvfb.kill(); };
	process.on("SIGTERM", stop);
	process.on("SIGINT", stop);
	while (chrome.exitCode === null && xvfb.exitCode === null) {
		await sleep(5_000);
		const targets = await fetch(`http://127.0.0.1:${port}/json/list`).then((r) => r.json()).catch(() => [] as any[]);
		const busy = Array.isArray(targets) && targets.some((t: any) => t.type === "page" && t.url !== "about:blank");
