...
if (!suppliedBase && !(await Bun.file(join(WEBSITE, ".next", "BUILD_ID")).exists())) {
...
while (
	...
	if (Date.now() > serverDeadline) throw new Error("Next.js website did not start");
	await sleep(100);
}
...
try {
	for (const shot of shots) {
		...
		try {
			...
			console.log(`${shot.name}: ${output} (${(Bun.file(output).size / 1024).toFixed(0)} KB)`);
		} finally {
			target.close();
