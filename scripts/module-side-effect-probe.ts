...
(Bun as any).spawnSync = (..._a: unknown[]) => {
	...
	return { exitCode: 0, success: true, stdout: Buffer.alloc(0), stderr: Buffer.alloc(0) } as never;
};
...
for (const m of modules) {
	...
	try {
		...
		failed.push({ module: m, error: String((e as Error)?.message || e).slice(0, 300) });
	}
}

await Bun.write(outPath, JSON.stringify({ scanned: modules.length, hits, failed }));
