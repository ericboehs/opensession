...
describe("GitHub Actions security", () => {
  ...
  test("pull request workflows are read-only and never use self-hosted runners", () => {
    for (const workflow of workflows) {
      ...
      expect(workflow.source, `${workflow.name} must grant only contents: read`).toMatch(
        /^permissions:\s*\n\s+contents:\s*read\s*$/m,
      );
      expect(workflow.source, `${workflow.name} must not grant write permissions`).not.toMatch(
        /^\s+[a-z-]+:\s*write\s*$/m,
      );
      expect(workflow.source, `${workflow.name} must not use repository secrets`).not.toContain(
        "secrets.",
      );
      expect(workflow.source, `${workflow.name} must not run public code on a self-hosted runner`).not.toContain(
        "self-hosted",
      );
    }
  });
  ...
  test("every external action is pinned to a full commit SHA", () => {
    for (const workflow of workflows) {
      for (const match of workflow.source.matchAll(/^\s*-?\s*uses:\s*([^\s#]+)(?:\s+#.*)?$/gm)) {
        const spec = match[1];
        if (spec.startsWith("./")) continue;
        const separator = spec.lastIndexOf("@");
        const ref = separator >= 0 ? spec.slice(separator + 1) : "";
        expect(ref, `${workflow.name}: ${spec} must be SHA-pinned`).toMatch(/^[0-9a-f]{40}$/);
      }
    }
  });
});
