...
test("refresh-file atomically supplies a current token", async () => {
  ...
  const server = Bun.serve({
    port: 0,
    fetch(request) {
      expect(request.headers.get("authorization")).toBe("Bearer exchange-lease");
      return new Response(token);
    },
  });
  const directory = mkdtempSync(join(tmpdir(), "opensession-id-token-"));
  const file = join(directory, "token");
  const child = Bun.spawn([process.execPath, "scripts/workload-identity-client.ts", "sandbox", "id-token", "--audience", "urn:test", "--refresh-file", file], {
    cwd: process.cwd(),
    env: {
      ...process.env,
