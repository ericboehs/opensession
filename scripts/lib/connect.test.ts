...
import { parseRunnerPortalRegistry, repairWindowsPath, resolveWindowsSchtasks, resolveWindowsShell, runnerExecCommand, runnerLaunchdPlist, runnerScheduledTaskXml, runnerSystemdUnit, serializeRunnerPortalRegistry, windowsPowerShellPath, windowsRunnerEnvironment, windowsSystem32 } from "./connect";
...
describe("Runner service definitions", () => {
	...
	test("a PATH missing System32 is repaired for child processes", () => {
		const repaired = repairWindowsPath("C:\\Users\\o\\.bun\\bin", "C:\\Windows");
		expect(repaired).toBe("C:\\Windows\\System32;C:\\Windows\\System32\\WindowsPowerShell\\v1.0;C:\\Windows\\System32\\Wbem;C:\\Users\\o\\.bun\\bin");
		// An empty PATH is the worst case and still has to come out usable.
		expect(repairWindowsPath("", "C:\\Windows").startsWith("C:\\Windows\\System32;")).toBe(true);
	});

	test("a healthy PATH is left exactly as it was, whatever its casing or trailing slashes", () => {
		const healthy = "C:\\WINDOWS\\system32\\;C:\\WINDOWS\\System32\\WindowsPowerShell\\v1.0;C:\\Windows\\System32\\Wbem;C:\\tools";
	...
	test("the Windows runner environment repairs PATH under whatever spelling it arrives in", () => {
		const env = windowsRunnerEnvironment({ Path: "C:\\nope", SystemRoot: "D:\\Win" });
		expect(env.Path).toBe("D:\\Win\\System32;D:\\Win\\System32\\WindowsPowerShell\\v1.0;D:\\Win\\System32\\Wbem;C:\\nope");
		expect(env.PATH).toBeUndefined();
		// No PATH at all in the parent still yields a working one.
		expect(windowsRunnerEnvironment({ SystemRoot: "C:\\Windows" }).Path).toContain("C:\\Windows\\System32");
	});
	...
	test("System32 binaries resolve from SystemRoot rather than PATH", () => {
		expect(windowsSystem32("schtasks.exe", "C:\\Windows")).toBe("C:\\Windows\\System32\\schtasks.exe");
		expect(windowsSystem32("schtasks.exe", "D:\\Win\\")).toBe("D:\\Win\\System32\\schtasks.exe");
		expect(windowsPowerShellPath("C:\\Windows")).toBe("C:\\Windows\\System32\\WindowsPowerShell\\v1.0\\powershell.exe");
	});
	...
	test("the shell prefers the known path, then pwsh, then the bare name", () => {
		...
		expect(resolveWindowsShell({ systemRoot: "C:\\Windows", exists: (p) => p === qualified, which: () => null })).toBe(qualified);
		expect(resolveWindowsShell({ systemRoot: "C:\\Windows", exists: () => false, which: (b) => (b === "pwsh.exe" ? "C:\\PS\\pwsh.exe" : null) })).toBe("C:\\PS\\pwsh.exe");
		// Last resort keeps today's behaviour rather than failing outright.
		expect(resolveWindowsShell({ systemRoot: "C:\\Windows", exists: () => false, which: () => null })).toBe("powershell.exe");
	});
	...
	test("schtasks resolves off SystemRoot, and says nothing was found rather than guessing", () => {
		...
		expect(resolveWindowsSchtasks({ systemRoot: "C:\\Windows", exists: (p) => p === qualified, which: () => null })).toBe(qualified);
		expect(resolveWindowsSchtasks({ systemRoot: "C:\\Windows", exists: () => false, which: (b) => (b === "schtasks.exe" ? "C:\\alt\\schtasks.exe" : null) })).toBe("C:\\alt\\schtasks.exe");
		expect(resolveWindowsSchtasks({ systemRoot: "C:\\Windows", exists: () => false, which: () => null })).toBeUndefined();
	});
	...
	test("the scheduled task names a fully qualified PowerShell, never a bare one", () => {
		const xml = runnerScheduledTaskXml("C:\\cli.ts", "C:\\bun.exe", "OFFICE\\owner", "D:\\Windows\\System32\\WindowsPowerShell\\v1.0\\powershell.exe");
		expect(xml).toContain("<Command>D:\\Windows\\System32\\WindowsPowerShell\\v1.0\\powershell.exe</Command>");
