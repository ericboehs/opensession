...
const SIGNALS: Signal[] = [
	{
		id: "raw-button",
		instead: "ui/button.tsx — carries the focus ring, press, and disabled states",
		exempt: ["ui/"],
	...
	},
	{
		...
		instead: "the semantic type scale — text-meta, text-label, text-body, text-item-title",
		exempt: ["styles/"],
	...
	},
	{
		...
		instead: "the radius scale — rounded-sm/md/lg/xl/2xl, rounded-control/row/popup",
		exempt: ["styles/"],
		count: (source) => {
			let n = 0;
...
export function auditCounts(): UiAuditResult {
	const budgets = JSON.parse(readFileSync(BUDGET_FILE, "utf8")) as Record<string, number>;
...
if (import.meta.main) {
	...
	if (only !== undefined) {
		const rows = [...(perFile.get(only) ?? new Map())].sort((a, b) => b[1] - a[1]);
		if (!rows.length) {
			console.error(`No occurrences of "${only}". Signals: ${SIGNALS.map((s) => s.id).join(", ")}`);
			process.exit(2);
		}
