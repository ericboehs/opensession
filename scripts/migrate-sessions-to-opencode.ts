...
const rows: Row[] = [];
for (const file of readdirSync(BACKSTAGE_CHATS_DIR)) {
  if (!file.endsWith(".json") || SKIP_FILES.has(file)) continue;
  let data: BackstageSessionFile;
  try {
    data = JSON.parse(readFileSync(`${BACKSTAGE_CHATS_DIR}/${file}`, "utf-8"));
  } catch {
    continue;
  }
  if (!data?.id) continue;
...
console.log(`Backstage session store: ${BACKSTAGE_CHATS_DIR}`);
