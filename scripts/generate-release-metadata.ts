...
function parseLock(lockPath: string): any {
  return JSON.parse(readFileSync(lockPath, "utf8").replace(/,(\s*[}\]])/g, "$1"));
}
...
function runtimePackageNames(lock: any, includeDev: boolean): Set<string> {
  ...
  for (const workspace of Object.values(lock.workspaces ?? {}) as any[]) {
    for (const name of Object.keys(workspace.dependencies ?? {})) roots.add(name);
    for (const name of Object.keys(workspace.optionalDependencies ?? {})) roots.add(name);
    if (includeDev) {
      for (const name of Object.keys(workspace.devDependencies ?? {})) roots.add(name);
    }
  }
  ...
  for (const entry of Object.values(lock.packages ?? {}) as LockEntry[]) {
    ...
    for (const group of [meta.dependencies, meta.optionalDependencies, meta.peerDependencies]) {
      for (const dependency of Object.keys(group ?? {})) current.add(dependency);
    }
          ...
          if (typeof pkg.name === "string" && typeof pkg.version === "string") {
            const raw = typeof pkg.license === "string"
              ...
              : Array.isArray(pkg.licenses)
                ? pkg.licenses.map((item: any) => typeof item === "string" ? item : item?.type).filter(Boolean).join(" OR ")
                : "";
...
export function spdxLicense(raw: string | undefined): string {
  ...
  return /^[A-Za-z0-9.+-]+(?:\s+(?:AND|OR|WITH)\s+[A-Za-z0-9.+-]+)*$/.test(value)
    ? value
    : "NOASSERTION";
}

function sha512Checksum(integrity: string | undefined): { algorithm: string; checksumValue: string }[] | undefined {
  if (!integrity?.startsWith("sha512-")) return undefined;
  return [{ algorithm: "SHA512", checksumValue: Buffer.from(integrity.slice(7), "base64").toString("hex") }];
}
...
export function generateReleaseMetadata(options: {
  ...
  const records = [...packages.values()].sort((a, b) =>
  ...
  const spdxPackages = records.map((pkg, index) => ({
    ...
    ...(sha512Checksum(pkg.integrity) ? { checksums: sha512Checksum(pkg.integrity) } : {}),
    externalRefs: [{
  ...
  const created = new Date(Number(process.env.SOURCE_DATE_EPOCH ?? "0") * 1000).toISOString();
  const sbom = {
  ...
  writeFileSync(join(options.outDir, "SBOM.spdx.json"), JSON.stringify(sbom, null, 2) + "\n");
  const inventory = [
    ...
    ...records.map((pkg) => `${pkg.name}\t${pkg.version}\t${pkg.license}\tpkg:npm/${pkg.name.replace("@", "%40")}@${pkg.version}`),
    "",
  ].join("\n");
...
if (import.meta.main) {
  ...
  const nodeModules = resolve(option("node-modules", join(dirname(lockPath), "node_modules"))!);
  ...
  if (!existsSync(nodeModules) || !statSync(nodeModules).isDirectory()) throw new Error(`node_modules not found: ${nodeModules}`);
  const result = generateReleaseMetadata({ lockPath, nodeModules, outDir, name, version, includeDev });
  console.log(`wrote SPDX SBOM and license inventory for ${result.packageCount} runtime packages to ${outDir}`);
}
