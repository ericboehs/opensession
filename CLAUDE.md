...
- **Agent awareness**: `runSessionPrompt` passes `reposNote` (built by `buildReposNote`) through `runAgent`; Claude gets it in the appended system prompt, and Codex gets it prepended to the turn prompt because the Codex SDK has no system-prompt hook. It lists primary + attached repos with their worktree paths so the agent cd's into the right isolated checkout. Only present when the session has attached repos.
