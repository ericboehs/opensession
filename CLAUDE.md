...
- **Agent awareness**: `runSessionPrompt` passes `reposNote` (built by `buildReposNote`) to `runClaude`, appended to the system prompt — it lists primary + attached repos with their worktree paths so the agent cd's into the right isolated checkout. Only present when the session has attached repos.
