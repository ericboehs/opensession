...
invoke models, or execute outbox effects. Long-running filesystem, network,
process, and model effects remain in the independently supervised executors.
Their active receipts do not hold the actor mailbox, so Stop, steering, and
fenced run events remain responsive.
