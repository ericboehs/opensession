...
| Project setup | Opt-in, credential-free provider snapshot/template | **Project preparation ready**. |
| Session durability | Docker snapshots, MicroVM memory snapshots, provider pause/archive | **Sandbox sleeping**. |
| Wake | Provider resume/restore plus `.agents/resume` | **Waking Sandbox**. |
| Services | Canonical Preview plus discovered ports | Multiple supervised **Portals**. |
...
Replace the singular Preview lifecycle with a session service supervisor.
It owns each service's process group, health checks, lifecycle, and cleanup.
Services survive an agent turn but are stopped or marked unavailable when the
session/Sandbox is destroyed.

The existing canonical repository app becomes the default Portal service; it
is not a competing product concept named Preview.

Maintain a session-local `.ports.conf` service registry. It records the stable
service name, sandbox-local port, description, state, process identity, and
optional default route. Open Session owns this generated state in the normal
flow; agents can inspect it but do not manually maintain it.
...
Expose these tools to eligible interactive code sessions:
...
For Daytona and Box, make the Open Session-owned Portal route the user path.
Do not make provider preview URLs part of the browser flow.
...
> app, call `opensession-portals.start_portal`, verify it with `list_portals`,
> then tell the user which Portal is ready.
...
Adopt one provider-neutral lifecycle:
...
While a Sandbox is Sleeping or Waking:
...
- a successful wake runs `.agents/resume`, restores the Portal relay/service
  state as appropriate, then drains the queue;
- a failed wake preserves the queue and offers Retry in the Sandbox popover.

## 8. Project preparation and acceleration

Keep Project preparation opt-in per project. It remains credential-free:
private `.env` files and other session authority are added only after shared
preparation state restores.
...
Before enabling arbitrary agent-created remote Portals, prove:
...
- Docker, MicroVM, Daytona, and Box pass the same lifecycle matrix.
