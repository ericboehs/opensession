 ...
 * recording, oldest-first replay, and the overflow contract. Live sends keep
 * going, resends turn partial). Pure in-memory; no host process involved.
 */
...
describe("TranscriptRelay", () => {
  ...
  test("stops recording past the byte budget but reports it", () => {
    ...
    // Recorded history is untouched by the rejected batch.
    expect(relay.replay().length).toBe(1);
    expect(relay.replay()[0].lines[0].uuid).toBe("a");
  });

  test("a small batch after overflow may still fit the remaining budget", () => {
