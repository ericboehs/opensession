...
function inGamut(c: Rgb): boolean {
	const e = 0.0005;
	return (
		c.r >= -e && c.r <= 1 + e && c.g >= -e && c.g <= 1 + e && c.b >= -e && c.b <= 1 + e
	);
}
...
function flipColor(c: Rgb): Rgb {
	...
	return { r: clamp(best.r) / 255, g: clamp(best.g) / 255, b: clamp(best.b) / 255, a: c.a };
}
...
export function adaptReportHtml(html: string, theme: ReportTheme): string {
	try {
		...
		if (theme === "light" || scheme === "dark") return injectBaseline(html, scheme);
