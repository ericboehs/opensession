...
function ctx(dir: string, over: Partial<WorkflowExecCtx> = {}): WorkflowExecCtx {
	return {
		runId: "wf-test-0001",
		sessionId: "bks-test",
		cwd: dir,
...
describe("mergeWorkflowAgents", () => {
	...
	it("refuses a dirty session worktree (nothing merged)", async () => {
		...
		const res = await mergeWorkflowAgents(ctx(dir), [{ seq: 0, branch: "wf-a" }]);
...
function doneEvents(text: string): StreamEvent[] {
	return [
		...
		{ type: "done", usage: { inputTokens: 5, outputTokens: 9, costUsd: 0, cacheReadTokens: 0, cacheCreationTokens: 0, contextTokens: 0 } },
	];
}
...
describe("write-agent execution", () => {
	...
	function writeReq(): WorkflowAgentRequest {
		return { seq: 0, prompt: "edit a.txt", opts: { write: true, label: "edit-a" } };
	}
	...
	it("a write agent that changes a file commits it and reports the diffstat", async () => {
		...
		const out = await workflowExecutor.execute(writeReq(), ctx(dir, { repo: "x", baseBranch: "main" }));
		expect(out.ok).toBe(true);
