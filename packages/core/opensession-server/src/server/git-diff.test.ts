...
describe("getSessionDiff", () => {
  test("coalesces concurrent reads of the same worktree", async () => {
	...
	test("scopes tracked and untracked files to the supplied paths", async () => {
		...
		expect(Bun.spawnSync(["git", "add", "."], { cwd: dir, env }).exitCode).toBe(0);
		expect(Bun.spawnSync(["git", "commit", "-m", "initial"], { cwd: dir, env }).exitCode).toBe(0);
	...
	test("keeps a timed-out computation coalesced until the underlying work settles", async () => {
		...
		const first = getSessionDiff("/virtual/diff", "main", exec, false, undefined, [], 10);
		await expect(first).rejects.toThrow("Git diff timed out");

		const timedOut = getSessionDiff("/virtual/diff", "main", exec, false, undefined, [], 10);
		expect(timedOut).toBe(first);
		await expect(timedOut).rejects.toThrow("Git diff timed out");

		releaseFirst();
		await Bun.sleep(10);
		const replacement = getSessionDiff("/virtual/diff", "main", exec, false, undefined, [], 1000);
