...
export async function assertFetchableUrl(raw: string): Promise<URL> {
	...
	for (const address of addresses) {
		if (isBlockedAddress(address)) {
			throw new Error(`Refusing to fetch ${host}: it resolves to the private address ${address}`);
		}
	}
	return url;
}
...
export async function htmlToText(html: string): Promise<{ title?: string; text: string }> {
	...
	const stripped = await new HTMLRewriter()
		...
		.on("h1, h2, h3, h4, h5, h6", { element: (e) => void e.after("\n", { html: true }) })
...
function loadPage(handle: string): { meta: CacheMeta; body: string } | undefined {
	try {
		const dir = cacheDir();
...
async function fetchChecked(
	...
	throw new Error(`Too many redirects (more than ${MAX_REDIRECTS}) from ${raw}`);
}
 ...
 */
export async function fetchWeb(raw: string, opts: FetchWebOptions = {}): Promise<FetchedPage> {
	const headChars = Math.max(200, Math.min(MAX_HEAD_CHARS, opts.headChars ?? DEFAULT_HEAD_CHARS));
	...
	if (buffer.byteLength > MAX_BODY_BYTES) {
		throw new Error(`${raw} exceeded the ${MAX_BODY_BYTES / 1024 / 1024}MB body limit`);
	}
...
export function readFetched(handle: string, opts: ReadFetchedOptions = {}): FetchedSlice {
