...
describe("session asset aliases", () => {
	...
	test("preserves descriptions across rewrites and removes them with files", async () => {
		await writeAsset(canonicalId, "report.html", Buffer.from("first"), "Q3 report");
		...
		expect((await listAssetsAcross([canonicalId]))[0]?.description).toBeUndefined();
	});
	...
	test("stores descriptions for filenames that overlap object properties", async () => {
		await writeAsset(canonicalId, "__proto__", Buffer.from("data"), "Prototype report");
		expect(await listAssetsAcross([canonicalId])).toMatchObject([
...
describe("S3-compatible asset storage", () => {
	...
	test("stores descriptions in object metadata and preserves them on rewrite", async () => {
		...
		await store.write(canonicalId, "reports/café.txt", Buffer.from("first"), "Café ✓");
		...
		expect((await store.read(canonicalId, "reports/café.txt"))?.data.toString()).toBe(
			"second",
		);
