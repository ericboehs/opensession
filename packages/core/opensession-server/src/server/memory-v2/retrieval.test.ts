...
describe("query extraction and ranking", () => {
	test("Actor queries retrieve the relevant facts and exclude implementation history", () => {
		const records = [
			record("actor-1", "Actor workers receive one mailbox message at a time.", { kind: "constraint" }),
			record("actor-2", "SessionActor owns the actor lifecycle and restart boundary.", { kind: "decision", tags: ["Actor"] }),
			record("actor-3", "The Actor test harness lives in session-kernel/actor-worker.test.ts.", { kind: "reference" }),
			...Array.from({ length: 25 }, (_, index) =>
				record(`noise-${index}`, `PR ${1000 + index} implemented UI cleanup and was deployed on August ${index + 1}.`, {
					kind: "status",
					updatedAt: "2026-08-22T11:59:00.000Z",
				}),
			),
		];
		const result = retrieveMemory(records, "Please look at the Actor implementation", {
			scopeKeys: SCOPES,
	...
	test("uses OR retrieval for meaningful terms instead of requiring every substring", () => {
		...
		const result = retrieveMemory(records, "What do we know about postgres kubernetes?", {
			scopeKeys: SCOPES,
			now: NOW,
		});
		expect(new Set(result.records.map(({ record }) => record.id))).toEqual(new Set(["postgres", "kube"]));
		expect(extractMemoryQueryTerms("What do we know about postgres kubernetes?")).toEqual([
			"postgres",
			"kubernetes",
		]);
	});
	...
	test("exact paths and symbols outrank broad lexical matches", () => {
		const records = [
			record("broad", "The actor worker mailbox has tests and restart handling.", {
				kind: "constraint",
				lastConfirmedAt: "2026-08-22T11:59:00.000Z",
			}),
			record("exact", "Mailbox ownership is documented in src/session-kernel/ActorWorker.ts.", {
				kind: "reference",
				updatedAt: "2025-01-01T00:00:00.000Z",
			}),
		];
		const ranked = rankMemoryRecords(records, "check src/session-kernel/ActorWorker.ts mailbox", {
			scopeKeys: SCOPES,
			now: NOW,
		});
	...
	test("scope specificity breaks otherwise equal matches repo before user before team", () => {
		const records = [
			record("team", "The deployment socket is /tmp/app.sock.", { scopeKey: "workspace" }),
			record("user", "The deployment socket is /tmp/app.sock.", { scopeKey: "user-kent" }),
			record("repo", "The deployment socket is /tmp/app.sock.", { scopeKey: "repo-opensession" }),
		];
		...
		expect(ranked.map(({ record }) => record.id)).toEqual(["repo", "user", "team"]);
	});
});
...
describe("eligibility and isolation", () => {
	test("excludes expired, superseded, archived, and explicitly expired records", () => {
		const records = [
			...
			record("ttl", "Actor TTL elapsed.", { expiresAt: "2026-08-22T11:59:59.000Z" }),
		...
		const result = retrieveMemory(records, "Actor", { scopeKeys: SCOPES, now: NOW });
	...
	test("never returns a relevant record outside the caller's scopes", () => {
		const records = [
			...
			record("secret", "Actor secret exact policy and credentials.", { scopeKey: "repo-other" }),
		];
...
describe("ambient budget and trust", () => {
	test("renders only pinned user-explicit or settings records by default", () => {
		const records = [
			record("explicit", "Use concise responses.", { tier: "pinned", sourceType: "user-explicit" }),
			record("settings", "Never use an em dash.", { tier: "pinned", sourceType: "settings" }),
			record("agent", "An agent inferred this preference.", { tier: "pinned", sourceType: "agent-verified" }),
			record("slack", "A Slack message stated this once.", { tier: "pinned", sourceType: "slack" }),
			record("retrievable", "This trusted fact is not pinned.", { sourceType: "user-explicit" }),
		];
		const result = renderAmbientMemory(records, { scopeKeys: SCOPES, now: NOW });
		expect(new Set(result.records.map((item) => item.id))).toEqual(new Set(["explicit", "settings"]));
	...
	test("supports an explicit caller trust policy without changing persisted records", () => {
		...
		expect(renderAmbientMemory([slack], { scopeKeys: SCOPES, now: NOW }).records).toHaveLength(0);
		expect(renderAmbientMemory([slack], {
			scopeKeys: SCOPES,
			now: NOW,
	...
	test("enforces the exact 2.5 KB total including the header, with no scope floor", () => {
		const records = Array.from({ length: 12 }, (_, index) =>
			record(`repo-${index}`, `Pinned repository constraint ${index}: ${"é".repeat(180)}`, {
				tier: "pinned",
				sourceType: "settings",
				kind: "constraint",
			}),
		...
		const one = records[0];
		const full = renderAmbientMemory([one], { scopeKeys: SCOPES, now: NOW, budgetBytes: 10_000 });
		const exactBytes = utf8Bytes(full.text);
		expect(renderAmbientMemory([one], {
			scopeKeys: SCOPES,
			now: NOW,
			budgetBytes: exactBytes,
		}).bytes).toBe(exactBytes);
		expect(renderAmbientMemory([one], {
			scopeKeys: SCOPES,
			now: NOW,
			budgetBytes: exactBytes - 1,
	...
	test("ambient rendering also enforces scope and expiry", () => {
		const records = [
			record("visible", "Visible pin.", { tier: "pinned", sourceType: "settings" }),
			record("other", "Other repo pin.", { scopeKey: "repo-other", tier: "pinned", sourceType: "settings" }),
			record("expired", "Expired pin.", { tier: "pinned", sourceType: "settings", expiresAt: NOW }),
		];
		const result = renderAmbientMemory(records, { scopeKeys: ["repo-opensession"], now: NOW });
