...
export interface RankedMemoryRecord<T extends RetrievalRecord = RetrievalRecord> {
	record: T;
	score: number;
	reasons: string[];
}

export interface MemoryRetrievalResult<T extends RetrievalRecord = RetrievalRecord>
...
export interface AmbientMemoryResult<T extends RetrievalRecord = RetrievalRecord>
	extends Omit<BudgetedText<T>, "items"> {
	records: T[];
}

const STOP_WORDS = new Set([
	"a", "about", "again", "all", "also", "am", "an", "and", "any", "are", "as", "at",
	"be", "been", "before", "but", "by", "can", "could", "current", "do", "does", "for",
	"from", "get", "go", "had", "has", "have", "here", "how", "i", "if", "in", "into", "is",
	"it", "its", "just", "know", "like", "look", "make", "me", "my", "need", "of", "ok", "on", "or",
	"our", "please", "should", "so", "some", "that", "the", "their", "them", "then", "there",
	"these", "they", "this", "to", "up", "us", "want", "was", "we", "were", "what", "when",
	...
	"change", "changes", "code", "done", "fix", "fixed", "implement", "implementation", "task",
]);
...
export function extractMemoryQueryTerms(query: string): string[] {
	return [...new Set(
		lexicalTokens(query).filter(
			(term) => term.length >= 2 && !STOP_WORDS.has(term) && !/^\d+$/.test(term),
		),
	)];
}

function queryIdentifiers(query: string): string[] {
	const quoted = [...query.matchAll(/[`"']([^`"']{2,})[`"']/g)].map((match) => match[1]);
	const symbolic = rawTokens(query).filter((token) => {
		const clean = token.replace(/^[./:@#$-]+|[./:@#$-]+$/g, "");
		if (clean.length < 2 || STOP_WORDS.has(clean.toLocaleLowerCase())) return false;
		return (
			/[A-Z]/.test(clean[0] ?? "") ||
	...
	return [...new Set([...quoted, ...symbolic].map(normalizedToken).filter(Boolean))];
}

function scopeKind(scopeKey: string): "repo" | "user" | "team" | "channel" | "other" {
...
function scopeScore(scopeKey: string, primaryRepoKey?: string): number {
	...
	switch (scopeKind(scopeKey)) {
		case "repo": return 0.65;
		case "user": return 0.45;
		case "team": return 0.25;
		case "channel": return 0.15;
		default: return 0;
	}
}
...
function timeMs(value: Date | string | number | undefined, fallback = 0): number {
...
function isSelectable(
	...
	return !record.expiresAt || timeMs(record.expiresAt, Number.NEGATIVE_INFINITY) > nowMs;
}

function recencyScore(timestamp: string | undefined, nowMs: number, halfLifeDays: number): number {
...
function fieldTokens(record: RetrievalRecord): {
	...
	for (const [tokens, weight] of [[summary, 3], [tags, 4], [details, 0.65]] as const) {
		for (const token of tokens) {
			weightedFrequency.set(token, (weightedFrequency.get(token) ?? 0) + weight);
		}
	}
...
export function rankMemoryRecords<T extends RetrievalRecord>(
	...
	for (const term of queryTerms) {
		documentFrequency.set(term, candidates.filter(({ fields }) => fields.all.has(term)).length);
	}
	const averageLength = candidates.reduce((sum, item) => sum + item.fields.length, 0) / candidates.length || 1;
	...
	for (const { record, fields } of candidates) {
		...
		for (const term of queryTerms) {
			...
			const denominator = tf + 1.2 * (0.25 + 0.75 * fields.length / averageLength);
		...
		const recency = recencyScore(record.updatedAt || record.createdAt, nowMs, 90) * 0.35;
		const coverage = matchedTerms / queryTerms.length;
		const score = exact + lexical * 8 + coverage * 2 + specificity + kind + freshness + recency;
		ranked.push({ record, score, reasons });
	}

	return ranked.sort((a, b) =>
		b.score - a.score ||
		timeMs(b.record.lastConfirmedAt || b.record.updatedAt || b.record.createdAt) -
			timeMs(a.record.lastConfirmedAt || a.record.updatedAt || a.record.createdAt) ||
...
export function retrieveMemory<T extends RetrievalRecord>(
	...
	const rendered = renderWithinByteBudget(ranked, {
		...
		limit: Math.min(RETRIEVED_MEMORY_LIMIT, Math.max(0, opts.limit ?? RETRIEVED_MEMORY_LIMIT)),
...
function ambientPriority(record: RetrievalRecord, primaryRepoKey?: string): number {
	return scopeScore(record.scopeKey, primaryRepoKey) * 10 + (KIND_SCORE[record.kind] ?? 0);
}
...
export function renderAmbientMemory<T extends RetrievalRecord>(
	...
	const eligible = records
		...
		.sort((a, b) =>
			ambientPriority(b, opts.primaryRepoKey) - ambientPriority(a, opts.primaryRepoKey) ||
