...
describe("legacy JSON migration", () => {
  test("preserves original text, maps lifecycle, and is idempotent without modifying JSON", async () => {
    ...
    const original = JSON.stringify({ entries: [
      { id: "old-1", text: longText, by: "Fable", at: "2026-02-01T10:00:00Z", supersedes: ["old-3"] },
      { id: "old-2", text: "A retired fact", archivedAt: "2026-02-02T10:00:00Z" },
      { id: "old-3", text: "A replaced fact", archivedAt: "2026-02-02T10:00:00Z", supersededBy: "old-1" },
    ] }, null, 2);
    ...
    try {
      ...
      expect(first).toMatchObject({ files: 1, discovered: 3, imported: 3, alreadyImported: 0, mapped: 3, skipped: 0, complete: true, errors: [] });
      const all = store.list({ states: ["active", "archived", "superseded"] }, { limit: 10 }).items;
      ...
      expect(all.find((item) => item.details === "A retired fact")?.state).toBe("archived");
      ...
      expect(second).toMatchObject({ imported: 0, alreadyImported: 3, mapped: 3, complete: true, errors: [] });
      expect(store.list({ states: ["active", "archived", "superseded"] }).items).toHaveLength(3);
  ...
  test("shares an existing exact record instead of duplicating it", async () => {
    ...
    await Bun.write(file, JSON.stringify({ entries: [{ id: "old-1", text: "Keep responses concise." }] }));
    const store = new MemoryStore(join(dir, "memory.db"));
    try {
      const existing = store.create({
        scopeKey: "workspace", summary: "Keep responses concise.", details: "Keep responses concise.",
        kind: "preference", tier: "pinned", source: { type: "user-explicit" },
	  });
      expect((await importLegacyMemoryDirectory(store, legacyDir)).imported).toBe(1);
  ...
  test("does not archive a native record when its deduplicated legacy alias is removed", async () => {
	...
	test("fails closed when one legacy file reuses an id", async () => {
		...
		await Bun.write(join(legacyDir, "workspace.json"), JSON.stringify({ entries: [
			...
			{ id: "same-id", text: "Different fact.", at: "2025-01-02T00:00:00Z" },
		] }));
		const store = new MemoryStore(join(dir, "memory.db"));
		try {
