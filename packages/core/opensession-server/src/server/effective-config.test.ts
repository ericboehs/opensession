...
const CATALOG = {
  ...
  brex: { type: "http", url: "https://brex.example", allowedUsers: ["Michiel", "Grant"] },
};
...
describe("explainMcpServers", () => {
  test("no allowlist: every configured server is listed and attributed", () => {
    const rows = explain({ included: { slack: {}, linear: {}, brex: {} }, gateUsers: ["Michiel"] });
  ...
  test("a server outside the allowlist says so, and is not dropped from the report", () => {
    ...
    expect(byName(rows, "linear").reason).toBe("outside this run's MCP allowlist");
    expect(byName(rows, "slack").reason).toContain("named by this run's MCP allowlist");
  });

  test("an allowedUsers miss names both the gate and who was tried", () => {
    const rows = explain({ included: { slack: {}, linear: {} }, gateUsers: ["Kent"] });
  ...
  test("an automation run (no user at all) reads as a gate miss, not an omission", () => {
    const rows = explain({ included: { slack: {}, linear: {} }, gateUsers: [] });
  ...
  test("an allowlist naming an unconfigured server is reported, not silently dropped", () => {
    const rows = explain({ scope: ["slack", "ghost"], included: { slack: {} } });
...
describe("describeStrippedTools", () => {
  ...
  test("money-movers are attributed to the confirm catalog on every run", () => {
    ...
    expect(refund!.ids).toEqual(["stripe_create_refund", "*_create_refund", "create_refund"]);
  ...
  test("automation denials are attributed to the automation catalog", () => {
    const denied = { mcp__plain__reply_to_thread: "read-only toward the customer" };
    const rows = describeStrippedTools(policy(denied), denied, STRIPE_CONFIRM_TOOLS);
  ...
  test("the engine's own native question tool is explained too", () => {
    const rows = describeStrippedTools(policy(), undefined, STRIPE_CONFIRM_TOOLS);
