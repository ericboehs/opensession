...
describe("sandbox start reliability", () => {
  test("recognizes bounded transport and service failures", () => {
    expect(isTransientSandboxStartError(Object.assign(new Error("fetch failed"), { cause: { code: "ECONNRESET" } }))).toBe(true);
    expect(isTransientSandboxStartError(Object.assign(new Error("provider unavailable"), { status: 503 }))).toBe(true);
  ...
  test("retries ensure exactly once and never loops", async () => {
    ...
    const provider = {
        ...
        calls += 1;
        if (calls === 1) throw Object.assign(new Error("network error"), { code: "ECONNRESET" });
        return { id: "sandbox-1", provider: "modal", workspace: "/workspace" };
      },
    } as any;
    const result = await ensureSandboxWithTransientRetry(
      provider,
      { sessionId: "session-1", repo: "opensession", mode: "code", cwd: "/tmp" },
      { delayMs: 0 },
    );
    ...
    await expect(
      ensureSandboxWithTransientRetry(
        alwaysDown,
        { sessionId: "session-2", repo: "opensession", mode: "code", cwd: "/tmp" },
        { delayMs: 0 },
      ),
