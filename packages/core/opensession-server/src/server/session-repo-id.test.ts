...
function withConfig(): void {
	...
	writeFileSync(
		...
		JSON.stringify({
			...
			repos: {
				app: { repo: APP_REPO, wtPrefix: "app", defaultBranch: "main", default: true },
...
describe("repoForPathOrNull", () => {
	...
	test("undefined instead of throwing for a path no repo owns", () => {
		withConfig();
		expect(repoForPathOrNull("/var/lib/opensession/scratch/os-123")).toBeUndefined();
...
describe("sessionRepoId", () => {
	...
	test("repo-less sessions resolve by path, whatever their mode", () => {
		...
		expect(sessionRepoId({ repo: undefined, worktreeDir: scratchDir })).toBeUndefined();
	});
});
