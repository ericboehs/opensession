...
function snapshotImages(sessionId: string, requestId: string, images: string[]): {
	dir: string;
	paths: string[];
} {
	...
	try {
		const paths = [...new Set(images)].slice(0, 10).map((source, index) => {
			if (!validFeaturedScreenshot(source)) {
				throw new Error(`Slack image is unavailable: ${basename(source) || "image"}`);
			}
...
export function snapshotPendingSlackImages(
	...
	if (!pending || pending.request.id !== requestId || pending.status !== "sending") {
...
export function claimPendingSlackComposer(sessionId: string, requestId: string): boolean {
	const pending = pendingSlackComposers.get(sessionId);
	if (!pending || pending.request.id !== requestId || pending.status !== "pending") return false;
	pending.status = "sending";
	return true;
}

export function restorePendingSlackComposer(sessionId: string, requestId: string): void {
	const pending = pendingSlackComposers.get(sessionId);
	if (pending?.request.id === requestId && pending.status === "sending") pending.status = "pending";
}
...
export function resendPendingSlackComposer(sessionId: string, send: (message: object) => void): void {
	const pending = pendingSlackComposers.get(sessionId);
	send({
		type: "slack_composer",
		sessionId,
