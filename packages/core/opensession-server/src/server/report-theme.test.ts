...
import { adaptReportHtml, authoredScheme, reportBaselineCss } from "./report-theme";
...
describe("authoredScheme", () => {
	...
	test("resolves a background written as a custom property", () => {
		...
		expect(
			authoredScheme(doc(":root{--paper:#fafafa}body{background:var(--paper)}")),
		).toBe("light");
	});
...
describe("adaptReportHtml — light", () => {
	...
	test("a dark document keeps its own scheme and gets the matching baseline", () => {
		const out = adaptReportHtml(doc("body{background:#0f1115;color:#eee}"), "light");
...
describe("adaptReportHtml — dark", () => {
	...
	test("keeps hue, so a signal colour still signals", () => {
		...
		const hueIsGreen = (hex: string) => {
			...
			return g > parseInt(hex.slice(1, 3), 16) && g > parseInt(hex.slice(5, 7), 16);
		};
	...
	test("flips named colours", () => {
		const flipped = authoredCss(adaptReportHtml(doc("body{background:white}"), "dark"));
		expect(flipped).not.toContain("white");
		expect(hexLuminance(flipped.match(/(#[0-9a-f]{6})/i)![1])).toBeLessThan(0.05);
	});
	...
	test("never rewrites a custom property whose name is a colour word", () => {
		...
		expect(flipped).toContain(".wrong{color:var(--red);background:var(--red-bg)}");
	...
	test("takes the document's own color-scheme with it", () => {
		const flipped = authoredCss(
			adaptReportHtml(doc(":root{color-scheme:light}body{background:#fff}"), "dark"),
		);
	...
	test("a document already in the target scheme is passed straight through", () => {
		...
		expect(authoredCss(adaptReportHtml(source, "dark"))).toBe(authoredCss(source));
	});
});
...
describe("adaptReportHtml — safety", () => {
	...
	test("the baseline leaves the root unpainted, so a document's own paper reaches the edges", () => {
		...
		expect(reportBaselineCss("dark")).not.toMatch(/(^|\})html\s*\{[^}]*background/);
	});
});
