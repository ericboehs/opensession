...
const stored: Map<string, StoredSuggestions> = (g.__replySuggestions ??= new Map());
...
function conversational(sessionId: string): boolean {
	...
	if (session.automation || session.goalId || (session as { desk?: boolean }).desk)
		return false;
	return true;
}

/** The row a client should currently see, or null. */
export function getReplySuggestions(sessionId: string): ReplySuggestion[] | null {
 ...
 */
export function maybeSuggestRepliesOnReturn(sessionId: string, user?: string): void {
...
export function parseSuggestions(raw: string | null): ReplySuggestion[] {
	if (!raw) return [];
	let text = raw.trim().replace(/^```[a-z]*\s*|\s*```$/g, "").trim();
	if (!text.startsWith("[")) {
