...
describe("Wafer provider", () => {
  test("keeps a lowercase public catalog", () => {
    expect(defaultPickerModelsForProvider("wafer")).toEqual(WAFER_PICKER_MODELS);
  ...
  test("builds Pi's provider catalog", () => {
    ...
    expect(catalog?.models.map((model) => model.id)).toEqual([...WAFER_PICKER_MODELS]);
  });

  test("exposes labels and efforts", () => {
    expect(piModelLabel("pi/wafer/deepseek-v4-flash-0731-fast")).toBe("DeepSeek V4 Flash");
    expect(modelEfforts("pi/wafer/glm-5.2")).toEqual([...waferModelEfforts("glm-5.2")]);
  });
});
