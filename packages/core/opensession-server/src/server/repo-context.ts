...
async function readLayout(repoPath: string): Promise<string[]> {
	...
	const out = await git(repoPath, ["ls-tree", "-d", "-r", "--name-only", "HEAD"]);
	if (!out) return [];
	const dirs = out
		.split("\n")
		.map((line) => line.trim())
...
export async function repoRoutingCatalog(): Promise<RepoCard[]> {
	...
	const key = repos.map((r) => `${r.id}:${r.repo}:${r.description || ""}`).join("|");
	if (cache.key === key && Date.now() - cache.at < CACHE_TTL_MS) return cache.cards;
...
export function renderRepoCatalog(cards: RepoCard[]): string {
	return cards
		.map((card) => {
			const lines = [`### ${card.id}${card.label && card.label !== card.id ? ` (${card.label})` : ""}`];
