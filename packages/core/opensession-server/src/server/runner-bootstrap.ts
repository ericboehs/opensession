...
export function parseRunnerBootstrapConfig(value: unknown): RunnerBootstrapConfig {
	const raw = value && typeof value === "object" && !Array.isArray(value) ? value as Record<string, unknown> : {};
	const ssh = (Array.isArray(raw.ssh) ? raw.ssh : []).flatMap((entry): SshBootstrapTarget[] => {
		...
		const port = typeof item.port === "number" && Number.isInteger(item.port) && item.port > 0 && item.port <= 65_535 ? item.port : 22;
		if (!id || !label || !host || !user || !fingerprint || !knownHostsPath || !ID.test(id) || !HOST.test(host) || !USER.test(user) || !fingerprint.startsWith("SHA256:")) return [];
		return [{ id, label, host, user, port, fingerprint, knownHostsPath, runnerCommand }];
	});
	const kubernetes = (Array.isArray(raw.kubernetes) ? raw.kubernetes : []).flatMap((entry): KubernetesBootstrapTarget[] => {
		const item = entry && typeof entry === "object" && !Array.isArray(entry) ? entry as Record<string, unknown> : {};
		const id = string(item.id, 64);
		const label = string(item.label, 160);
...
async function output(args: string[], input?: string): Promise<{ code: number; stdout: string; stderr: string }> {
	const proc = Bun.spawn(args, { stdin: input === undefined ? "ignore" : "pipe", stdout: "pipe", stderr: "pipe" });
	if (input !== undefined) {
		proc.stdin?.write(input);
		proc.stdin?.end();
	}
	const [stdout, stderr, code] = await Promise.all([new Response(proc.stdout).text(), new Response(proc.stderr).text(), proc.exited]);
	return { code, stdout: stdout.slice(0, 8_000), stderr: stderr.slice(0, 8_000) };
}

async function verifySshFingerprint(target: SshBootstrapTarget): Promise<void> {
	if (!existsSync(target.knownHostsPath)) throw new Error("The configured SSH known-hosts file is unavailable.");
	const keys = await output(["ssh-keyscan", "-T", "10", "-p", String(target.port), target.host]);
	if (keys.code !== 0 || !keys.stdout.trim()) throw new Error("SSH connection or host-key lookup failed.");
	const fingerprints = await output(["ssh-keygen", "-lf", "-"], keys.stdout);
	if (fingerprints.code !== 0 || !fingerprints.stdout.includes(target.fingerprint)) {
		throw new Error("The SSH host fingerprint does not match the configured pin.");
	}
}

function connectCommand(command: string, server: string, code: string, name: string): string {
...
export async function bootstrapSshRunner(targetId: string, code: string): Promise<{ target: SshBootstrapTarget; phase: "pairing" }> {
	...
	const result = await output([
		...
		"-o", `UserKnownHostsFile=${target.knownHostsPath}`, "-p", String(target.port),
		`${target.user}@${target.host}`, remote,
	]);
	if (result.code !== 0) throw new Error(result.stderr.trim() || "The SSH Runner bootstrap command failed.");
...
export async function bootstrapKubernetesRunner(targetId: string, code: string): Promise<{ target: KubernetesBootstrapTarget; phase: "pairing" }> {
	...
	const exec = await output([
		...
		...(target.container ? ["--container", target.container] : []), "--", "sh", "-lc", command,
	]);
	if (exec.code !== 0) throw new Error(exec.stderr.trim() || "The Kubernetes Runner bootstrap command failed.");
	return { target, phase: "pairing" };
}
