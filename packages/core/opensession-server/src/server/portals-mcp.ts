...
import { getPreviewStatus, getSandboxPreviewStatus, recipeStartOptions, sandboxPreviewIdentityContext } from "./preview";
...
async function startPortalForContext(ctx: PortalsMcpContext, dir: string, sandbox: Sandbox | null, input: PortalStartInput): Promise<string> {
	const session = ctx.runner();
	if (session?.runner) {
	...
	const portal = sandbox
		? await startSandboxPortalService({ sessionId: ctx.sessionId, sandbox, ...input, env: sandboxPortalEnv(ctx, sandbox) })
		: await startPortalService({ sessionId: ctx.sessionId, worktreeDir: dir, ...input });
		...
		name: "opensession-portals", version: "1.0.0", tools: [
			tool("start_portal", "Start a supervised HTTP or WebSocket service in this session workspace. Open Session allocates a port when omitted, sets PORT and PORTAL_URL, waits for it to listen, and returns its authenticated Portal URL. Never use an upstream URL: Portals expose only this session's process.", {
				...
				try {
					...
					const sandbox = runner?.runner ? null : await ctx.sandbox({ wake: true });
					if (!sandbox && !runner?.runner && ctx.hasSandbox()) return result("Could not start Portal: this session's Sandbox is unavailable.");
					const status = await portalStatus(ctx, dir, sandbox);
					const recipe = status.portalRecipes.find((candidate) => candidate.id === args.name);
					return result(await startPortalForContext(ctx, dir, sandbox, recipe ? recipeStartOptions(recipe) : args));
				} catch (error) { return result(`Could not start Portal: ${(error as Error).message}`); }
			}),
			tool("start_declared_portal", "Start a repository-declared Portal by its ID. Use this instead of copying its command: Open Session applies the trusted command, port contract, readiness timeout, and sandbox workload identity from .agents/portals.json.", {
				id: z.string(),
			}, async ({ id }: { id: string }) => {
				const dir = workspace(ctx); if (dir instanceof Error) return result(dir.message);
				try {
					...
					if (!sandbox && !runner?.runner && ctx.hasSandbox()) return result("Could not start Portal: this session's Sandbox is unavailable.");
					const status = await portalStatus(ctx, dir, sandbox);
					const recipe = status.portalRecipes.find((candidate) => candidate.id === id);
					if (!recipe) return result(`Could not start Portal: declared Portal '${id}' was not found.`);
			...
			tool("restart_portal", "Restart one supervised Portal using its registered command and port. Repository-declared Portals are refreshed from their trusted recipe before restart.", { name: z.string() }, async ({ name }: { name: string }) => {
				const dir = workspace(ctx); if (dir instanceof Error) return result(dir.message);
				try {
					const runner = ctx.runner();
					const sandbox = runner?.runner ? null : await ctx.sandbox({ wake: true });
					if (!sandbox && !runner?.runner && ctx.hasSandbox()) return result("Could not restart Portal: this session's Sandbox is unavailable.");
					const status = await portalStatus(ctx, dir, sandbox);
