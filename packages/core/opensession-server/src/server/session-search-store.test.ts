...
describe("SessionSearchStore", () => {
	...
	test("recency decay outranks older equal-relevance record", () => {
		const s = new SessionSearchStore(":memory:");
		s.upsert(
			rec({ id: "session:old", question: "worktree cleanup", ts: NOW - 300 * DAY }),
		);
		s.upsert(
			rec({ id: "session:new", question: "worktree cleanup", ts: NOW - 2 * DAY }),
		);
	...
	test("hostile query strings don't throw", () => {
		...
		expect(() => s.search('NEAR( "unclosed OR AND NOT *', { now: NOW })).not.toThrow();
