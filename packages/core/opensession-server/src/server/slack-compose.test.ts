...
afterEach(() => {
	for (const [sessionId, pending] of pendingSlackComposers) {
		if (pending.status === "sending") restorePendingSlackComposer(sessionId, pending.request.id);
...
describe("Slack composer lifecycle", () => {
	...
	test("blocks until the human sends", async () => {
		...
		expect(sendPendingSlackComposer("os-test", request!.id, { id: "C1", name: "engineering" })).toBe(true);
		expect(await result).toEqual({ status: "sent", channel: { id: "C1", name: "engineering" } });
	});
	...
	test("allows only one pending composer per session", () => {
		...
		expect(() => openSlackComposer("os-test", {})).toThrow("already has a Slack composer open");
	});
