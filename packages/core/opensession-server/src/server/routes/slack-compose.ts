...
import { configuredSlackChannels, defaultSlackChannel, slackChannelsPayload } from "./slack-channels";
import type { RouteContext } from "./context";

function targetChannel(value: unknown) {
	const channels = configuredSlackChannels();
	const wanted = typeof value === "string" ? value.trim().replace(/^#/, "") : "";
	return channels.find((channel) => channel.id === wanted || channel.name.toLowerCase() === wanted.toLowerCase())
		|| (!wanted ? channels.find((channel) => channel.id === defaultSlackChannel(channels)) : undefined);
}
...
export async function handleSlackComposeRoutes(ctx: RouteContext): Promise<Response | undefined> {
	...
	const undoMatch = ctx.path.match(/^\/api\/sessions\/([^/]+)\/slack-composer\/undo$/);
	if (undoMatch && ctx.req.method === "POST") {
		const caller = ctx.authUser?.login || ctx.authUser?.name;
		if (!caller) {
			return Response.json({ error: "Sign in to undo this Slack message" }, { status: 401 });
		}
		const body = await ctx.req.json().catch(() => ({}));
	...
	if (ctx.req.method === "GET") return Response.json(await slackChannelsPayload(ctx));
	const body = await ctx.req.json().catch(() => ({}));
	const caller = ctx.authUser?.login || ctx.authUser?.name;
	if (!caller) {
		return Response.json({ error: "Sign in to review this Slack message" }, { status: 401 });
	...
	if (!pending || pending.request.id !== requestId) {
		return Response.json({ error: "Slack composer is no longer open" }, { status: 409 });
	}
	if (ctx.req.method === "DELETE") {
		if (!cancelPendingSlackComposer(sessionId, requestId)) {
			return Response.json({ error: "Slack message is already being sent" }, { status: 409 });
		}
		return Response.json({ status: "cancelled" });
	}

	const channel = targetChannel(body?.channel);
	if (!channel) return Response.json({ error: "Choose a configured Slack channel" }, { status: 400 });
	const message = typeof body?.message === "string" ? body.message.trim() : "";
	if (message.length > 500) return Response.json({ error: "Slack message must be 500 characters or fewer" }, { status: 400 });
	const requestedScreenshots = Array.isArray(body?.screenshots) ? body.screenshots : [];
	if (requestedScreenshots.some((path: unknown) => typeof path !== "string" || !validFeaturedScreenshot(path))) {
		return Response.json({ error: "One or more Slack images are no longer available" }, { status: 400 });
	}
	const screenshots = [...new Set<string>(requestedScreenshots)].slice(0, 10);
	if (!message && screenshots.length === 0) {
		return Response.json({ error: "Write a message or add an image first" }, { status: 400 });
	}
	...
	try {
		...
		const permalink = ts ? await slackPermalink(channel.id, ts, slackToken) : undefined;
		...
		return Response.json({ error: error?.message || "Couldn't send to Slack" }, { status: 502 });
	}
}
