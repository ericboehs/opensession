...
const usageCache: Map<string, CodexAccountUsage> = ((globalThis as any)
...
async function probeCodexUsageUnlocked(
	...
	try {
		...
		return usageError(`Couldn't start Codex usage probe: ${error?.message || error}`);
	}
					...
					while (newline >= 0) {
						...
						if (message.id === 1) {
							if (message.error) {
								finish(
									usageError(
										message.error.message || "Codex usage initialization failed",
									),
								);
								return;
							}
