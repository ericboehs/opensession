...
afterEach(() => {
  if (previous === undefined) delete process.env.OPENSESSION_WORKSPACE_SECRETS_STORE;
...
describe("workspace secrets", () => {
  test("creates, rotates, resolves and deletes an opaque 0600 secret", () => {
    ...
    expect(statSync(process.env.OPENSESSION_WORKSPACE_SECRETS_STORE!).mode & 0o777).toBe(0o600);
    ...
    expect(readFileSync(process.env.OPENSESSION_WORKSPACE_SECRETS_STORE!, "utf-8")).not.toContain(
      "daytona-secret",
    );
