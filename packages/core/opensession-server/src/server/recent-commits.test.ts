...
describe("parseCommitLog", () => {
	...
	it("counts an insertion-only commit, and one that only removes", () => {
		const rows = parseCommitLog(
			entry(["aaa", "A", "a@example.com", "2026-08-14T10:00:00+00:00", "Add"], " 1 file changed, 9 insertions(+)") +
				entry(["bbb", "B", "b@example.com", "2026-08-14T09:00:00+00:00", "Drop"], " 1 file changed, 2 deletions(-)"),
			{ id: "opensession" },
		);
	...
	it("keeps a subject that contains a separator character", () => {
		const [row] = parseCommitLog(
			entry(["ddd", "D", "d@example.com", "2026-08-14T10:00:00+00:00", `Fix${FIELD}this`]),
			{ id: "opensession" },
		);
