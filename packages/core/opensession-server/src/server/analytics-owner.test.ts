...
describe("session owner attribution", () => {
	test("a person owns their own session", () => {
		...
		expect(resolveOwnerRef(store(kent), kent)).toEqual({ kind: "person", meta: kent });
	});

	test("a delegated worker is credited to whoever delegated it", () => {
		const kent = session(HUMAN, "Kent");
		const worker = session(CHILD, workerActor(HUMAN), { parentSessionId: HUMAN });
		expect(resolveOwnerRef(store(kent, worker), worker)).toEqual({ kind: "person", meta: kent });
	});

	test("credit survives a chain of machine sessions", () => {
		const kent = session(HUMAN, "Kent");
		const worker = session(CHILD, workerActor(HUMAN), { parentSessionId: HUMAN });
		const nested = session(GRANDCHILD, agentActor(CHILD), { parentSessionId: CHILD });
	...
	test("automations and reviews keep their existing buckets", () => {
		const auto = session(HUMAN, "docs-sync (automation)", { automationName: "docs-sync" });
		expect(resolveOwnerRef(store(auto), auto)).toEqual({ kind: "automation", name: "docs-sync" });
