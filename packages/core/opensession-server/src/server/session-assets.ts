...
class LocalAssetStore implements AssetStore {
	...
	async list(sessionId: string): Promise<SessionAssetFile[]> {
		...
		const walk = (d: string, prefix: string, depth: number) => {
			...
			for (const entry of entries) {
				...
				if (
					!prefix &&
					(entry.name === ASSET_METADATA_FILE || entry.name === ASSET_METADATA_TEMP)
				)
					continue;
	...
	async open(sessionId: string, relPath: string): Promise<SessionAssetStream | null> {
		const { abs, rel } = resolveAssetPath(sessionId, relPath);
	...
	async read(sessionId: string, relPath: string): Promise<SessionAssetContent | null> {
		const { abs, rel } = resolveAssetPath(sessionId, relPath);
		if (!existsSync(abs)) return null;
		const st = statSync(abs);
		if (!st.isFile()) return null;
...
function encodeDescription(description: string | undefined): string | undefined {
	const clean = description?.trim().slice(0, 500);
	return clean ? encodeURIComponent(clean) : undefined;
}

function decodeDescription(metadata: Record<string, string> | undefined): string | undefined {
...
export class S3AssetStore implements AssetStore {
	kind = "s3" as const;
	private listCache = new Map<string, { at: number; files: SessionAssetFile[] }>();
	...
	private key(sessionId: string, relPath: string): { key: string; rel: string } {
		const rel = safeAssetPath(relPath);
	...
	async write(
		...
		await this.client.send(
			new PutObjectCommand({
				...
				...(encoded ? { Metadata: { [DESCRIPTION_METADATA_KEY]: encoded } } : {}),
			}),
		);
		...
		return {
			...
			description: description === undefined ? keptDescription : description.trim().slice(0, 500) || undefined,
		};
	}
	...
	async list(sessionId: string): Promise<SessionAssetFile[]> {
		...
		const files = await mapLimited(objects, 12, async (object) => {
			...
			try {
				const head = await this.client.send(
					new HeadObjectCommand({ Bucket: this.config.bucket, Key: object.key }),
				);
	...
	async delete(sessionId: string, relPath: string): Promise<boolean> {
		...
		do {
			...
			for (const object of page.Contents || []) {
				if (object.Key === key || object.Key?.startsWith(prefix)) keys.add(object.Key);
			}
			continuationToken = page.IsTruncated ? page.NextContinuationToken : undefined;
		...
		for (let index = 0; index < all.length; index += 1000) {
			const result = await this.client.send(
				new DeleteObjectsCommand({
					Bucket: this.config.bucket,
					Delete: { Objects: all.slice(index, index + 1000).map((Key) => ({ Key })) },
				}),
			);
...
let remoteStoreCache:
	| { signature: string; store: S3AssetStore }
	| undefined;
...
export async function writeAsset(
	...
	const file = await primaryStore().write(sessionId, relPath, data, description);
