...
describe("PR contract ownership", () => {
	test("keeps provider-neutral contracts out of host implementations", async () => {
		...
		for (const name of infoContracts) {
			expect(contract).toMatch(new RegExp(`export (?:interface|type) ${name}\\b`));
			expect(info).not.toMatch(new RegExp(`export (?:interface|type) ${name}\\b`));
		}
	...
	test("preserves the legacy type export paths", async () => {
		...
		expect(info).toMatch(/export type \{[\s\S]*PrDetails[\s\S]*\} from "\.\/pr-contract"/);
		expect(host).toContain('export type { PrHostCapabilities } from "./pr-contract"');
		expect(stack).toContain('export type { PrStack, PrStackLayer } from "./pr-contract"');
	});
});
