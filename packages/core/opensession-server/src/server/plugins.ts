...
function validateFeed(entry: unknown, index: number, errors: string[]): void {
	...
	const map = items.map;
	if (!isObject(map) || typeof map.id !== "string" || typeof map.title !== "string") {
...
export function validateManifest(input: unknown): ManifestResult {
	...
	if (input.mcpServers !== undefined) {
		if (!isObject(input.mcpServers)) errors.push("mcpServers must be an object");
		else {
	...
	if (input.automations !== undefined) {
		if (!Array.isArray(input.automations)) errors.push("automations must be an array");
...
export function parseManifest(text: string): ManifestResult {
	...
	try {
		...
		return { errors: [`${PACKAGE_MANIFEST_FILE} is not valid JSON: ${(e as Error).message}`] };
	}
...
export function packagesStorePath(root?: string): string {
	return root ? `${root}/.opensession-plugins.json` : statePath(".opensession-plugins.json");
}
...
export function packagesCheckoutDir(root?: string): string {
	return root ? `${root}/.opensession-plugins` : statePath(".opensession-plugins");
}
...
export function recordInstalledPackage(pkg: InstalledPackage, root?: string): void {
	const packages = listInstalledPackages(root).filter((p) => p.name !== pkg.name);
	packages.push(pkg);
