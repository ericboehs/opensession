...
describe("isBlockedAddress", () => {
	...
	test("allows ordinary public addresses", () => {
		for (const ip of ["8.8.8.8", "1.1.1.1", "140.82.121.4", "172.32.0.1", "100.63.255.255", "2606:4700::1111"]) {
...
describe("assertFetchableUrl", () => {
	test("refuses a non-http scheme", async () => {
		await expect(assertFetchableUrl("file:///etc/passwd")).rejects.toThrow(/http/i);
		await expect(assertFetchableUrl("ftp://example.com")).rejects.toThrow(/http/i);
	});

	test("refuses a literal private address without needing DNS", async () => {
		await expect(assertFetchableUrl("http://127.0.0.1:3850/api/sessions")).rejects.toThrow(/private/i);
		await expect(assertFetchableUrl("http://169.254.169.254/latest/meta-data/")).rejects.toThrow(/private/i);
		await expect(assertFetchableUrl("http://[::1]/")).rejects.toThrow(/private/i);
	});
...
describe("htmlToText", () => {
	...
	test("reads the title", async () => {
		const { title } = await htmlToText("<html><head><title>  A Page  </title></head><body>x</body></html>");
...
describe("fetch → handle → read", () => {
	...
	async function withStubbedFetch<T>(body: string, contentType: string, fn: () => Promise<T>): Promise<T> {
	...
	test("offset reads walk the body without re-fetching", async () => {
		const page = await withStubbedFetch("0123456789".repeat(100), "text/plain", () =>
			fetchWeb("https://example.com/plain", { refresh: true, headChars: 200 }),
		);
