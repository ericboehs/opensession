...
const { cachedSandboxPortalOwner, cacheSandboxPortals, dropCachedSandboxPortals, sleepingSandboxPortalStatus } = await import("./sandbox-portals");
...
describe("sleeping Sandbox Portal cache", () => {
	test("keeps metadata readable without retaining a live URL", () => {
		cacheSandboxPortals("bks-test", "sbx-test", [{ name: "Demo", key: "PORTAL_DEMO_PORT", port: 4400, running: true, pids: [], previewUrl: "https://preview.test:20000", description: "A test service", state: "awake", managed: true }]);
