...
describe("historical public prefixes", () => {
	...
	test("keeps ordinary page loads canonical", () => {
		...
		expect(shouldRedirectLegacyPublicPath("GET", null, "/session/os-1")).toBe(true);
	});

	test("does not redirect APIs, mutations, or upgrades", () => {
		expect(shouldRedirectLegacyPublicPath("GET", null, "/api/sessions")).toBe(false);
		expect(shouldRedirectLegacyPublicPath("POST", null, "/prompt")).toBe(false);
		expect(shouldRedirectLegacyPublicPath("GET", "websocket", "/ws")).toBe(false);
	});
});
