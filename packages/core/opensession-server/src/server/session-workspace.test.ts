...
describe("settleProvisionalNames", () => {
  test("renames a workspace still wearing the session's raw key", () => {
    const ws = workspace("C0BE8KVFBEX-1786887720.562319");
    settleProvisionalNames([slack({ workspaceId: ws.id })]);
    expect(getWorkspace(ws.id)?.name).toBe("Fix sidebar hover states");
  });

  test("leaves it alone while the title is still that key", () => {
    const key = "C0BE8KVFBEX-1786887720.562319";
    const ws = workspace(key);
    settleProvisionalNames([slack({ workspaceId: ws.id, title: key })]);
    expect(getWorkspace(ws.id)?.name).toBe(key);
  });

  test("never overwrites a name someone chose", () => {
    const ws = workspace("Hover states");
    settleProvisionalNames([slack({ workspaceId: ws.id })]);
  ...
  test("leaves a workspace named after a git branch alone", () => {
    const ws = workspace("fix-sidebar-hover");
