...
export function registerPrChecksAgentWait(input: {
	...
	if (!sessionId) return { ok: false, error: "Current session id is required." };
	if (!repo) return { ok: false, error: "Repository id is required for a PR wait." };
...
export async function handleAgentWait(
	...
	if (!details) {
		const next = { ...wait, lastError: "No pull request found for the branch yet." };
		deps.schedule(next, nextPrPoll(next, now));
		return "rescheduled";
	}
	...
	await deps.deliver(wait, `PR ${wait.repo}#${details.number} checks settled. ${result}`);
	return "delivered";
}
