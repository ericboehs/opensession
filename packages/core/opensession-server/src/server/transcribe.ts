...
async function transcribeHosted(
	...
	if (typeof json.text !== "string") throw new Error("no text in transcription response");
...
async function transcribeLocal(audio: Blob, ext: string): Promise<string> {
	...
	try {
		...
		const ffmpeg = Bun.spawn(
			["ffmpeg", "-y", "-i", inPath, "-ar", "16000", "-ac", "1", "-f", "wav", wavPath],
...
export async function localWhisperAvailable(): Promise<boolean> {
	return (await Bun.file(WHISPER_CLI).exists()) && (await Bun.file(WHISPER_MODEL).exists());
}
...
export async function transcribeAudio(
	...
	if (await localWhisperAvailable()) {
		try {
			return { text: await transcribeLocal(audio, ext), provider: "whisper.cpp" };
		} catch (e: any) {
			errors.push(`whisper.cpp: ${e?.message || e}`);
		}
	} else {
		errors.push("whisper.cpp: binary or model missing (see src/server/transcribe.ts header)");
	}

	throw new Error(`transcription failed — ${errors.join("; ") || "no provider configured"}`);
}
