...
describe("validateManifest", () => {
	...
	test("a package with no artifacts is not a package", () => {
		expect(errorsOf({ name: "empty", version: "1.0.0", description: "nothing" })).toContainEqual(
			expect.stringContaining("at least one artifact"),
		);
	});
	...
	test("credential blocks take references, never values", () => {
		expect(errorsOf(withServer({ type: "http", url: "https://x.example/mcp", headers: { Authorization: "Bearer sk-live-123" } })))
			.toContainEqual(expect.stringContaining("must be a ${NAME} reference"));
		expect(errorsOf(withServer({ command: "node", args: ["server.js"], env: { TOKEN: "sk-live-123" } })))
			.toContainEqual(expect.stringContaining("must be a ${NAME} reference"));
		// A reference with anything around it is a value with a reference in it.
		expect(errorsOf(withServer({ command: "node", env: { TOKEN: "Bearer ${TOKEN}" } })))
			.toContainEqual(expect.stringContaining("must be a ${NAME} reference"));
		expect(errorsOf(withServer({ command: "node", env: { TOKEN: "${TOKEN}" } }))).toEqual([]);
	});

	test("a token hiding in the URL is refused too", () => {
		expect(errorsOf(withServer({ type: "http", url: "https://mcp.example/mcp?token=abc123" })))
			.toContainEqual(expect.stringContaining("no query string"));
		expect(errorsOf(withServer({ type: "http", url: "https://user:pw@mcp.example/mcp" })))
	...
	test("a server needs a url or a command, and http means http", () => {
		...
		expect(errorsOf(withServer({ type: "http", url: "ftp://mcp.example/mcp" }))).toContainEqual(
	...
	test("allowedUsers is refused", () => {
		expect(errorsOf(withServer({ type: "http", url: "https://x.example/mcp", allowedUsers: ["michiel"] })))
	...
	test("a package cannot enable or self-edit its own automation", () => {
		const enabled = {
			...VALID,
			automations: [{ id: "weekly", automation: { name: "n", prompt: "p", enabled: true } }],
		};
		expect(errorsOf(enabled)).toContainEqual(expect.stringContaining("cannot enable"));
		const selfImprove = {
			...VALID,
			automations: [{ id: "weekly", automation: { name: "n", prompt: "p", selfImprove: true } }],
		};
		expect(errorsOf(selfImprove)).toContainEqual(expect.stringContaining("self-editing"));
	});

	test("an automation needs a name and a prompt", () => {
		const bare = { ...VALID, automations: [{ id: "weekly", automation: { name: "n" } }] };
		expect(errorsOf(bare)).toContainEqual(expect.stringContaining("prompt is required"));
	});
...
describe("the ledger", () => {
	...
	test("recording twice replaces rather than duplicates", () => {
		...
		recordInstalledPackage(entry({ version: "1.1.0", updatedAt: "2026-08-17T00:00:00.000Z" }), root);
