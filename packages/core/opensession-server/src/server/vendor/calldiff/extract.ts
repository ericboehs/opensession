...
import type { CallStep, FunctionIndex, FunctionInfo, SourceLocation } from "./core";
...
function addFunction(index: FunctionIndex, info: FunctionInfo) {
	...
	if (!existing || (!existing.exported && info.exported)) index.set(key, { ...info, key });
}
...
function tsLocation(file: ts.SourceFile, node: ts.Node): SourceLocation {
	...
	return { file: file.fileName, line: start, ...(end > start ? { endLine: end } : {}) };
}

function hasExport(node: ts.Node): boolean {
	return Boolean(ts.getModifiers(node as ts.HasModifiers)?.some((m) => m.kind === ts.SyntaxKind.ExportKeyword));
}

function tsMemberKey(expr: ts.Expression, className: string | null): string | null {
	if (ts.isIdentifier(expr)) return expr.text;
	if (expr.kind === ts.SyntaxKind.ThisKeyword) return className;
	if (ts.isPropertyAccessExpression(expr)) {
		const prop = expr.name.text;
		if (expr.expression.kind === ts.SyntaxKind.ThisKeyword && className) return `${className}.${prop}`;
		if (ts.isIdentifier(expr.expression)) return `${expr.expression.text}.${prop}`;
		return className ? `${className}.${prop}` : prop;
	}
	return null;
}

function hookKey(node: ts.CallExpression, file: ts.SourceFile, base: string): string {
	if (!/^(?:useEffect|useLayoutEffect|useInsertionEffect|useMemo|useCallback|useImperativeHandle)$/.test(base)) return base;
	const deps = node.arguments[1];
	return deps && ts.isArrayLiteralExpression(deps) ? `${base}(${compact(deps.getText(file), 64)})` : base;
}

function tsSteps(file: ts.SourceFile, root: ts.Node, className: string | null, index: FunctionIndex): CallStep[] {
	...
	const visit = (node: ts.Node) => {
		...
		if (ts.isCallExpression(node)) {
			...
			if (raw) {
				const children = node.arguments.flatMap((arg) =>
					ts.isArrowFunction(arg) || ts.isFunctionExpression(arg) ? tsSteps(file, arg.body, className, index) : [],
				);
				steps.push({
					type: "call",
			...
			for (const arg of node.arguments) {
				if (!ts.isArrowFunction(arg) && !ts.isFunctionExpression(arg)) ts.forEachChild(arg, visit);
			}
			return;
		}
		if (ts.isNewExpression(node)) {
			const raw = tsMemberKey(node.expression, null);
			if (raw) steps.push({ type: "call", key: `new ${raw}`, ...tsLocation(file, node.expression) });
			return;
		}
		if (ts.isJsxElement(node) || ts.isJsxSelfClosingElement(node)) {
			...
			const children = ts.isJsxElement(node)
				? node.children.flatMap((child) => tsSteps(file, child, className, index))
				: [];
			if (isComponent) {
				steps.push({ type: "call", key: tag, ...tsLocation(file, opening.tagName), ...(children.length ? { children } : {}) });
				return;
			}
		}
...
function tsSignature(node: ts.SignatureDeclarationBase, file: ts.SourceFile): string {
...
function extractTypeScript(record: SourceRecord, index: FunctionIndex) {
	const kind = /\.[cm]?tsx$/i.test(record.path)
		...
		: /\.[cm]?jsx$/i.test(record.path)
			...
			: /\.[cm]?js$/i.test(record.path)
				? ts.ScriptKind.JS
				: ts.ScriptKind.TS;
	const file = ts.createSourceFile(record.path, record.content, ts.ScriptTarget.Latest, true, kind);
	const add = (key: string, node: ts.FunctionLikeDeclaration, exported: boolean, className: string | null) => {
		if (!node.body) return;
		addFunction(index, {
			key,
	...
	const visit = (node: ts.Node, exported = false, className: string | null = null) => {
			...
			for (const member of node.members) {
				if ((ts.isMethodDeclaration(member) || ts.isConstructorDeclaration(member)) && member.body) {
					...
					if (name === "constructor") {
						...
						if (info) addFunction(index, { ...info, key: `new ${node.name.text}`, label: `new ${node.name.text}${tsSignature(member, file)}` });
					}
				}
			}
			return;
...
function offsetLocation(path: string, starts: number[], node: SyntaxNode): SourceLocation {
	const line = lineAt(starts, node.from);
	const endLine = lineAt(starts, node.to);
	return { file: path, line, ...(endLine > line ? { endLine } : {}) };
}

function rustCallKey(node: SyntaxNode, source: string, owner: string | null, types: Map<string, string>): string | null {
	if (node.name === "Identifier" || node.name === "BoundIdentifier") return rustText(source, node);
	if (node.name === "FieldExpression") {
		...
		return type ? `${type}.${rustText(source, field)}` : `${objectName}.${rustText(source, field)}`;
	}
	if (node.name === "ScopedIdentifier") {
		...
		if (!name) return null;
		return name === "new" && scope ? `new ${scope}` : scope ? `${scope}.${name}` : name;
	}
	return null;
}

function rustSteps(path: string, source: string, starts: number[], root: SyntaxNode, owner: string | null, types: Map<string, string>, index: FunctionIndex): CallStep[] {
	const steps: CallStep[] = [];
	const walk = (node: SyntaxNode) => {
		if (node !== root && (node.name === "FunctionItem" || node.name === "ClosureExpression" || node.name === "ImplItem")) return;
		if (node.name === "IfExpression") {
			const children = rustChildren(node);
			const blocks = children.filter((child) => child.name === "Block");
			const condition = children.find((child) => !["if", "else", "Block", "IfExpression"].includes(child.name));
			steps.push({
				type: "branch",
				key: `if:${condition ? compact(rustText(source, condition)) : ""}`,
				label: condition ? `if (${compact(rustText(source, condition))})` : "if",
				...offsetLocation(path, starts, condition ?? node),
				children: blocks[0] ? rustSteps(path, source, starts, blocks[0], owner, types, index) : [],
			});
			if (blocks[1]) steps.push({ type: "branch", key: "else", label: "else", ...offsetLocation(path, starts, blocks[1]), children: rustSteps(path, source, starts, blocks[1], owner, types, index) });
			return;
		}
		if (node.name === "CallExpression") {
			const callee = rustChildren(node)[0];
			const key = callee ? rustCallKey(callee, source, owner, types) : null;
			if (key) steps.push({ type: "call", key, ...offsetLocation(path, starts, callee!) });
		}
...
function rustParams(source: string, params: SyntaxNode | null): { label: string; types: Map<string, string> } {
	...
	for (const param of params ? rustChildren(params) : []) {
		...
		const nameNode = rustChild(param, "BoundIdentifier") ?? rustChild(param, "Identifier");
		const typeNode = rustChildren(param).find((child) => /Type$/.test(child.name));
		const name = nameNode ? rustText(source, nameNode) : "_";
		names.push(name);
		if (nameNode && typeNode) types.set(name, compact(rustText(source, typeNode)).replace(/^&(?:mut )?/, ""));
	}
...
function extractRust(record: SourceRecord, index: FunctionIndex) {
	...
	const starts = [0];
	for (let i = 0; i < record.content.length; i++) if (record.content.charCodeAt(i) === 10) starts.push(i + 1);
	const add = (node: SyntaxNode, owner: string | null, trait: string | null, scope: string) => {
		const nameNode = rustChild(node, "BoundIdentifier") ?? rustChild(node, "Identifier");
		...
		const info: FunctionInfo = {
			...
			steps: rustSteps(record.path, record.content, starts, body, scopedOwner, params.types, index),
			exported: Boolean(rustChild(node, "Vis")) || !name.startsWith("_"),
		};
		addFunction(index, info);
		if (scopedOwner && name === "new") addFunction(index, { ...info, key: `new ${scopedOwner}` });
		if (trait) {
			const scopedTrait = `${scope}${trait}`;
			addFunction(index, { ...info, key: `${scopedTrait}.${name}`, label: `${scopedTrait}.${name}${params.label}` });
		}
	};
	const visitItems = (root: SyntaxNode, scope = "") => {
		for (const node of rustChildren(root)) {
			...
			if (node.name === "ImplItem") {
				const typeNodes = rustChildren(node).filter((child) => child.name === "TypeIdentifier");
				const owner = typeNodes.at(-1) ? rustText(record.content, typeNodes.at(-1)!) : null;
				const trait = typeNodes.length > 1 ? rustText(record.content, typeNodes[0]!) : null;
				const list = rustChild(node, "DeclarationList");
				if (owner && list) for (const item of rustChildren(list)) if (item.name === "FunctionItem") add(item, owner, trait, scope);
				continue;
			}
			if (node.name === "ModItem") {
				const nameNode = rustChild(node, "BoundIdentifier") ?? rustChild(node, "Identifier") ?? rustChild(node, "TypeIdentifier");
				const list = rustChild(node, "DeclarationList");
				if (nameNode && list) visitItems(list, `${scope}${rustText(record.content, nameNode)}::`);
			}
		}
	};
...
interface ReToken { text: string; start: number; end: number; line: number }
...
function reTokens(source: string): ReToken[] {
	...
	while (i < source.length) {
		...
		if (ident) { tokens.push({ text: ident, start: i, end: i + ident.length, line }); i += ident.length; continue; }
...
function matching(tokens: ReToken[], start: number, open: string, close: string): number {
	let depth = 0;
...
function reCallSteps(path: string, tokens: ReToken[], start: number, end: number, index: FunctionIndex): CallStep[] {
	const steps: CallStep[] = [];
	const ignored = new Set(["if", "else", "switch", "for", "while", "let", "module", "open", "include"]);
	for (let i = start; i < end; i++) {
		const token = tokens[i]!;
		if (token.text === "<" && /^[A-Z]/.test(tokens[i + 1]?.text ?? "")) {
			let key = tokens[++i]!.text;
			while (tokens[i + 1]?.text === "." && /^[A-Za-z_$]/.test(tokens[i + 2]?.text ?? "")) { key += `.${tokens[i + 2]!.text}`; i += 2; }
		...
		let key = token.text;
		let cursor = i;
		while (tokens[cursor + 1]?.text === "." && /^[A-Za-z_$]/.test(tokens[cursor + 2]?.text ?? "")) { key += `.${tokens[cursor + 2]!.text}`; cursor += 2; }
...
function extractReScript(record: SourceRecord, index: FunctionIndex) {
	...
	const scan = (start: number, end: number, prefix = "", baseDepth = 0) => {
		...
		for (let i = start; i < end; i++) {
			...
			if (text === "module" && /^[A-Z]/.test(tokens[i + 1]?.text ?? "") && tokens[i + 2]?.text === "=" && tokens[i + 3]?.text === "{") {
			...
			let equal = nameAt + 1;
			while (equal < end && tokens[equal]!.text !== "=" && tokens[equal]!.line <= tokens[i]!.line + 3) equal++;
			...
			if (blockBody) {
				...
				for (let cursor = bodyStart; cursor < end; cursor++) {
					...
					if ([")", "]", "}"].includes(value)) nested--;
					if (nested === 0 && (value === ";" || (cursor > bodyStart && ["let", "module"].includes(value)))) {
						bodyEnd = cursor;
						break;
					}
				}
			...
			const params = compact(tokens.slice(equal + 1, arrow).map((token) => token.text).join(""), 64);
			addFunction(index, {
				key,
