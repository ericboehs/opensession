...
describe("vendored calldiff extractors", () => {
	...
	test("keeps duplicate short function names from different files", () => {
		...
		expect(trees.map((tree) => tree.entry).sort()).toEqual(["a.ts::run", "b.ts::run"]);
	});
	...
	test("keeps adjacent expression-bodied ReScript functions separate", () => {
		const index = buildFunctionIndex([{
			path: "Math.res",
			content: "let one = x => first(x)\nlet two = x => second(x)\nlet three = x => third(x)",
		}]);
		expect([...index.keys()]).toEqual(["Math.res::one", "Math.res::two", "Math.res::three"]);
	...
	test("ignores multiline ReScript template contents and preserves later lines", () => {
		...
		expect(info?.steps.find((step) => step.type === "call" && step.key === "save")?.line).toBe(4);
	});

	test("caps complete TypeScript signature labels", () => {
		const params = Array.from({ length: 30 }, (_, index) => `value${index}: VeryLongTypeName${index}`).join(", ");
		const index = buildFunctionIndex([{
			path: "large.ts",
