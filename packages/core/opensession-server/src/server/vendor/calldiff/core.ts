...
export type CallStep =
	...
	| ({ type: "branch"; key: string; label: string; children: CallStep[] } & SourceLocation);
	...
	for (const step of steps) {
		...
		if (step.type === "branch") {
			...
			nodes.push({
				...
				children: expandSteps(step.children, index, depth, maxDepth, visiting, budget),
			});
			continue;
		}
		nodes.push(expandCall(step.key, index, depth, maxDepth, visiting, budget, step.children, step));
	}
	return nodes;
}
...
function expandCall(
	...
	const location = depth === 0 && info?.line
	...
	if (info && visiting.has(resolved)) {
		return {
			...
			children: inlineChildren
				? expandSteps(inlineChildren, index, depth + 1, maxDepth, visiting, budget)
				: [],
		};
	}
	if (info) visiting.add(resolved);
	const children = [
		...(info ? expandSteps(info.steps, index, depth + 1, maxDepth, visiting, budget) : []),
		...(inlineChildren
			? expandSteps(inlineChildren, index, depth + 1, maxDepth, visiting, budget)
			: []),
	];
...
export function buildCallTree(entry: string, index: FunctionIndex, maxDepth: number, maxNodes = 800): CallNode {
	return buildCallTreeResult(entry, index, maxDepth, maxNodes).tree;
}

function buildCallTreeResult(entry: string, index: FunctionIndex, maxDepth: number, maxNodes: number): { tree: CallNode; truncated: boolean } {
...
function mark(node: CallNode, status: "added" | "removed"): FlowNode {
	return { ...node, status, children: node.children.map((child) => mark(child, status)) };
}

function diffChildren(before: CallNode[], after: CallNode[]): FlowNode[] {
	const n = before.length;
	const m = after.length;
	const lengths = Array.from({ length: n + 1 }, () => Array<number>(m + 1).fill(0));
	for (let i = n - 1; i >= 0; i--) {
		for (let j = m - 1; j >= 0; j--) {
			lengths[i]![j] = before[i]!.key === after[j]!.key
				? lengths[i + 1]![j + 1]! + 1
...
function focused(node: FlowNode): FlowNode | null {
	const children = node.children.map(focused).filter((child): child is FlowNode => child !== null);
...
function functionSignature(info: FunctionInfo | undefined): string {
	return info ? `${info.label}[${info.steps.map(stepSignature).join(",")}]` : "<missing>";
}
...
export function buildChangedTrees(
	...
	const changed = [...keys].filter(
		(key) => !key.includes("::new ") && functionSignature(before.get(key)) !== functionSignature(after.get(key)),
	);
	const candidates = changed
		.sort((a, b) => {
			const exported = Number(Boolean(after.get(b)?.exported || before.get(b)?.exported)) -
	...
	let nodes = 0;
	let truncated = Boolean(before.truncated || after.truncated) || changed.length > candidates.length || changed.length > maxTrees;
	...
	for (const entry of candidates.slice(0, maxTrees)) {
		...
		const oldResult = before.has(entry) ? buildCallTreeResult(entry, before, maxDepth, remaining) : null;
		const newResult = after.has(entry) ? buildCallTreeResult(entry, after, maxDepth, remaining) : null;
		if (oldResult?.truncated || newResult?.truncated) truncated = true;
		const oldTree = oldResult?.tree ?? { key: entry, label: after.get(entry)?.label ?? entry, kind: "call" as const, children: [] };
		const newTree = newResult?.tree ?? { key: entry, label: before.get(entry)?.label ?? entry, kind: "call" as const, children: [] };
