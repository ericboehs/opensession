...
const unsupported = (what: string) =>
	({ error: `${what} isn't supported on code.storage — it has no pull requests, only branches` }) as const;
...
function branchesFor(repoId: string, maxAgeMs = BRANCHES_TTL): Promise<CsBranch[]> {
	const hit = branchesCache.get(repoId);
	if (hit && Date.now() - hit.ts < maxAgeMs) return Promise.resolve(hit.branches);
...
function splitMessage(message: string | undefined): { headline: string; body: string } {
	const text = (message || "").trim();
	const nl = text.indexOf("\n");
	if (nl === -1) return { headline: text, body: "" };
	return { headline: text.slice(0, nl).trim(), body: text.slice(nl + 1).trim() };
}
...
function buildFiles(diff: CsBranchDiff): PrFile[] {
	return [
		...
		...(diff.filtered_files || []).map((f) => ({ path: f.path, additions: 0, deletions: 0 })),
...
async function fetchDetails(branch: string, repoId: string): Promise<PrDetails | null> {
	...
	return {
		...
		additions: diff.stats?.additions ?? files.reduce((n, f) => n + f.additions, 0),
		deletions: diff.stats?.deletions ?? files.reduce((n, f) => n + f.deletions, 0),
		changedFiles: (diff.stats?.files ?? diff.files.length) + (diff.filtered_files?.length || 0),
		reviewDecision: "",
...
function cachedFetch<T>(
	...
	if (!fresh && hit && Date.now() - hit.ts < TTL) return Promise.resolve(hit.data);
...
export const csPrHost: PrHost = {
	...
	getPrDetails: (branch, repo) =>
		cachedFetch(detailsCache, detailsInflight, cacheKey(repo, branch), false, () =>
	...
	async mergePr(branch, opts, repo, credential) {
		try {
			const base = await defaultBranchOf(repo);
			if (branch === base) return { error: "That's the default branch — nothing to merge" };
			const head = await findBranch(repo, branch);
			if (!head) return { error: "No PR found for this branch" };
			if (opts.method === "rebase")
				return { error: "Rebase merges aren't supported on code.storage — use squash or merge" };
	...
	async closePr(branch, repo, credential) {
		try {
			const base = await defaultBranchOf(repo);
			if (branch === base) return { error: "That's the default branch — it can't be closed" };
			const head = await findBranch(repo, branch);
			if (!head) return { error: "No PR found for this branch" };
	...
	async listOpenPrs(repo, opts) {
		try {
			...
			const branches = (await branchesFor(repo))
				...
				.slice(0, opts.limit);
			let failed = 0;
			const rows = await mapLimit(branches, 4, async (b): Promise<BulkPr | null> => {
				try {
			...
			console.warn(`[cs-pr] listOpenPrs ${repo} failed: ${errorMessage(e).slice(0, 200)}`);
			return null;
		}
	},
