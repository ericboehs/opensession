...
export function parseCommitRecord(
	...
	const [sha, shortSha, author, email, date, title, ...bodyParts] = head.split(FIELD);
	...
	return {
		...
		...(body ? { body: body.length > BODY_MAX ? `${body.slice(0, BODY_MAX)}…` : body } : {}),
		author: author || "",
...
async function onDefaultBranch(dir: string, defaultBranch: string, sha: string): Promise<boolean> {
...
export async function lookupCommit(
	...
	if (cached) {
		if (cached.commit.onDefaultBranch || Date.now() - cached.branchCheckedAt < BRANCH_TTL_MS)
...
async function resolve(
	...
	if (known) {
		...
		return {
			...known,
			onDefaultBranch: await onDefaultBranch(repo.repo, known.defaultBranch, known.sha),
		};
	}
	const ordered = [
	...
	for (const repo of ordered) {
		...
		return {
			...found,
			defaultBranch,
			onDefaultBranch: await onDefaultBranch(repo.repo, defaultBranch, found.sha).catch(
				() => false,
			),
		};
	}
