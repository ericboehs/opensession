...
describe("bankWorkingState", () => {
	...
	it("banks tracked edits, untracked files and unpushed commits, then verifies", async () => {
		...
		const dirty = (await $`git -C ${work} status --porcelain`.quiet()).text().trim();
		const unpushed = (
		...
		if (!bankDir) return;
		expect(readFileSync(join(bankDir, "tracked.patch"), "utf8")).toContain("+edited");
		expect(existsSync(join(bankDir, "untracked.tar.gz"))).toBe(true);
		const verify = await $`git -C ${work} bundle verify ${join(bankDir, "unpushed.bundle")}`
			.quiet()
			.nothrow();
		expect(verify.exitCode).toBe(0);
		const meta = JSON.parse(readFileSync(join(bankDir, "metadata.json"), "utf8"));
