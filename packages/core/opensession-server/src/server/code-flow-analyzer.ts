...
export function analyzeCodeFlow(input: CodeFlowAnalysisInput): CodeFlowResult {
	...
	for (const pair of input.pairs) {
		const language = languageFor(pair.path) ?? (pair.oldPath ? languageFor(pair.oldPath) : null);
		if (language) languages.add(language);
		// Normalize a rename onto the new path so unchanged functions still match.
		if (pair.before !== null) before.push({ path: pair.path, content: pair.before });
		if (pair.after !== null) after.push({ path: pair.path, content: pair.after });
	}
	const changed = buildChangedTrees(buildFunctionIndex(before), buildFunctionIndex(after));
	return {
		repo: input.repo,
		base: input.base,
		head: input.head,
