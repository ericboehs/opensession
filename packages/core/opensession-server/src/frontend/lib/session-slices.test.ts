...
function session(id: string, over: Partial<UnifiedSession> = {}): UnifiedSession {
	return {
		id,
...
describe("settledOverrides", () => {
	...
	test("an unarchive is held until a poll that started after it", () => {
		expect(
			settledOverrides(
				slices({ live: [], liveAt: 1000, locallyUnarchived: new Map([["a", 3000]]) }),
			).unarchived,
		).toEqual([]);
		expect(
			settledOverrides(
				slices({ live: [], liveAt: 4000, locallyUnarchived: new Map([["a", 3000]]) }),
			).unarchived,
		).toEqual(["a"]);
	});
});
