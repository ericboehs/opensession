...
import { memoryNeedsReview, memorySourceLabel, memoryState, memorySummary } from "./memory-v2";

describe("memory v2 compatibility helpers", () => {
	test("keeps legacy entries readable", () => {
		const entry = { id: "old", text: "Legacy fact", by: "Alice", at: "2026-01-01T00:00:00Z" };
	...
	test("derives expiry and review state from lifecycle timestamps", () => {
		...
		expect(memoryState(entry, Date.parse("2026-01-03T00:00:00Z"))).toBe("expired");
