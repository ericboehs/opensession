...
describe("refTone", () => {
	test("a terminal PR reads by its ending, not its checks", () => {
		...
		expect(
			refTone(
				ref({ state: "MERGED", checks: { total: 1, passed: 0, failed: 1, pending: 0 } }),
			),
		).toBe("purple");
	});
...
describe("refState", () => {
	...
	test("counts pending checks instead of collapsing them to Open", () => {
		...
		expect(
			refState(
				ref({ isDraft: true, checks: { total: 2, passed: 0, failed: 0, pending: 2 } }),
			),
...
describe("refChipText", () => {
	test("drops the repo hint inside the session's own repo", () => {
		...
		expect(refChipText(ref({ repo: "tella-mac", number: 14 }), "tella-fusion")).toBe(
			"tella-mac #14",
		);
...
describe("summarizePrSeries", () => {
	...
	test("a landed series says so", () => {
		...
		expect(
			summarizePrSeries([merged("tella-fusion", "a"), merged("tella-mac", "b")]),
