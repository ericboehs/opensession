...
export function revealDiffFile(root: HTMLElement | null, path: string, attempt = 0): void {
	if (!root) return;
	const file = root.querySelector<HTMLElement>(`[data-diff-file="${CSS.escape(path)}"]`);
	if (!file) {
