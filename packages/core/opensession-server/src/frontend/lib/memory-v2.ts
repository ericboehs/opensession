...
export function memoryState(entry: MemoryRecordDto, now = Date.now()): MemoryState {
...
export async function updateMemoryRecord(
	scopeKey: string,
	id: string,
	patch: Partial<Pick<MemoryRecordDto, "summary" | "details" | "kind" | "tags" | "expiresAt">>,
): Promise<{ entry: MemoryRecordDto }> {
	return request(`/memory/${encodeURIComponent(id)}`, {
		method: "PATCH",
		body: { scopeKey, ...patch },
...
export async function fetchMemoryPage(input: {
	...
	return request(`/memory?${params.toString()}`, { label: "Failed to fetch memory" });
}

export async function readMemoryRecord(scopeKey: string, id: string): Promise<{ entry: MemoryRecordDto }> {
...
export async function permanentlyDeleteMemory(scopeKey: string, id: string): Promise<void> {
	const params = new URLSearchParams({ scopeKey, confirm: "true" });
	await request<void>(`/memory/${encodeURIComponent(id)}?${params.toString()}`, {
		method: "DELETE",
