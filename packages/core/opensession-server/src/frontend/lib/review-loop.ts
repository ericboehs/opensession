...
export function reviewLoopResult(session: SessionReviewState): ReviewLoopResult | undefined {
	if (!session.prNumber || session.prState !== "OPEN" || !session.prOsReview) return undefined;
