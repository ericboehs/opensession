...
describe("pickUnreadWorkspaceSession", () => {
	test("opens the unread tab with the newest activity", () => {
		const older = session({ id: "older", lastActivity: "2026-08-20T11:00:00.000Z" });
		const newer = session({ id: "newer", lastActivity: "2026-08-20T12:00:00.000Z" });
		expect(pickUnreadWorkspaceSession([older, newer], null, READS)?.id).toBe("newer");
	});

	test("ignores the selected tab and spawned workers", () => {
		const selected = session({ id: "selected", lastActivity: "2026-08-20T13:00:00.000Z" });
		const parent = session({ id: "older", lastActivity: "2026-08-20T11:00:00.000Z" });
		...
		expect(
			pickUnreadWorkspaceSession([selected, parent, worker], selected.id, READS)?.id,
		).toBe(parent.id);
	});

	test("uses a worker when the workspace has no parent tab", () => {
		const worker = session({ id: "worker", parentSessionId: "missing" });
		expect(pickUnreadWorkspaceSession([worker], null, READS)?.id).toBe(worker.id);
	});
});
