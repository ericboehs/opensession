...
export class WsCommandOutbox {
  ...
  put(message: WSClientMessage): boolean {
    ...
    const currentBytes = this.allItems().reduce((sum, stored) => sum + utf8Bytes(stored), 0);
  ...
  retireLegacy(id: string): boolean {
    ...
    const existed = this.allItems().some((item) => item.message.requestId === id);
    if (!existed) return false;
    if (this.isLegacyId(id) && !this.write(this.retiredKey(id), { requestId: id }))
      return false;
    try { this.store.removeItem(this.itemKey(id)); } catch {}
  ...
  adoptLegacyPrefix(oldPrefix: string): void {
    ...
    for (let index = 0; index < this.store.length; index++) {
      ...
      if (!key || key === marker || (key !== oldPrefix && !key.startsWith(`${oldPrefix}:`))) continue;
      const suffix = key.slice(oldPrefix.length);
      const value = this.store.getItem(key);
      if (value != null) copies.push([`${this.keyPrefix}${suffix}`, value, key]);
    }
    try {
  ...
  private allItems(): StoredItem[] {
    const retired = new Set(
      this.records<{ requestId: string }>(":retired:").map((item) => item.requestId),
    );
  ...
  private legacy(): LegacyStored | undefined {
    ...
    return value?.version === 1 && Array.isArray(value.items) ? value : undefined;
  }
  ...
  private migrateAggregate(): void {
    ...
    try {
      ...
      for (const item of legacy.items) {
        const id = requestId(item?.message);
        if (!id || retired.has(id) || !Number.isFinite(item?.createdAt)) continue;
...
export function wsCommandOutboxForScope(scope: string): WsCommandOutbox {
  ...
  if (!outbox) {
    outbox = new WsCommandOutbox(storage(), () => Date.now(), `${KEY_PREFIX}:${normalized}`);
    const priorScoped = scope.trim().toLowerCase().replace(/[^a-z0-9:._-]/g, "_");
    outbox.adoptLegacyPrefix(`${KEY_PREFIX}:${priorScoped}`);
    try {
      const legacyUser = globalThis.localStorage?.getItem("opensession-user") || "anonymous";
