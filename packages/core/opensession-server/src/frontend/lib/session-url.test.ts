...
describe("internalUrlTarget", () => {
	test("reads a session id out of both path shapes", () => {
		...
		expect(internalUrlTarget(`${HOST}/workspace/${WS}/session/${ID}`)?.sessionId).toBe(ID);
	});

	test("tolerates the legacy path prefixes and a trailing slash", () => {
		expect(internalUrlTarget(`${HOST}/opensession/session/${ID}/`)?.sessionId).toBe(ID);
		expect(internalUrlTarget(`${HOST}/backstage/session/${ID}`)?.sessionId).toBe(ID);
	});
...
describe("pastedSessionId", () => {
	...
	test("a link inside a larger paste is left alone", () => {
		...
		expect(pastedSessionId(`${HOST}/session/${ID} ${HOST}/session/${ID}`)).toBeUndefined();
	});
	...
	test("anything that is not a session link is left alone", () => {
		expect(pastedSessionId(`${HOST}/automations/auto-01a006d8-eddd-7000-bca2-b010caf2d8e7`)).toBeUndefined();
		expect(pastedSessionId("https://github.com/tellahq/opensession/pull/92")).toBeUndefined();
