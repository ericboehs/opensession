...
test("persists before returning and delivers each session in order", async () => {
	...
	const first = outbox.enqueue({ sessionId: "s1", content: "first", files: [{ name: "a", path: "/tmp/a" }], contextSessions: ["s2"], busyMode: "steer" });
	const second = outbox.enqueue({ sessionId: "s1", content: "second" });
	expect(JSON.stringify(storage.getItem("opensession-prompt-outbox:v1:test"))).toContain(first.clientId);
	await outbox.flush();
...
test("keeps a retryable failed head item and exposes it for retry or editing", async () => {
	...
	await outbox.flush();
	expect(outbox.list("s1")[0]).toMatchObject({ clientId: item.clientId, state: "pending", attempts: 1, error: "offline" });
	fail = false;
...
test("marks a rejected delivery failed without losing its editable payload", async () => {
	...
	const item = outbox.enqueue({ sessionId: "missing", content: "keep this", user: "Ada" });
	await outbox.flush();
	expect(outbox.list()).toEqual([
		expect.objectContaining({ clientId: item.clientId, state: "failed", content: "keep this", user: "Ada" }),
	]);
...
test("resumes a persisted send after the tab that started it closes", async () => {
	...
	storage.setItem(
		...
		JSON.stringify({
			version: 1,
			items: [{ clientId: "stable-id", sessionId: "s1", content: "resume", state: "sending", attempts: 0, createdAt: 1, nextAttemptAt: 1 }],
		}),
	);
	const outbox = new PromptOutbox({
		storage,
...
test("does not retain an item in memory when durable storage rejects it", () => {
	...
	expect(() => outbox.enqueue({ sessionId: "s1", content: "keep in composer" })).toThrow(
		"quota exceeded",
	);
	expect(outbox.list()).toEqual([]);
	outbox.dispose();
