...
describe("matchFocusTarget", () => {
	test("a number picks its own PR, not the first one in the repo", () => {
		expect(matchFocusTarget(targets(), { repo: "gitops", number: 955 })?.key).toBe(
			"gitops grid-pool",
		);
		expect(matchFocusTarget(targets(), { repo: "gitops", number: 961 })?.key).toBe(
			"gitops node-taints",
		);
	});
	...
	test("falls back to the branch when the number is unknown", () => {
		expect(
			matchFocusTarget(targets(), { repo: "gitops", branch: "node-taints" })?.key,
	...
	test("a number the panel doesn't offer lands on the repo's primary tab", () => {
		...
		expect(matchFocusTarget(targets(), { repo: "gitops", number: 12 })?.key).toBe(
			"gitops",
		);
	});
