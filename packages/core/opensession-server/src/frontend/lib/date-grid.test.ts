...
describe("bounds", () => {
	...
	it("clamps to the nearest bound", () => {
		expect(clampDay("2026-01-01", "2026-07-19", "2026-08-17")).toBe("2026-07-19");
		expect(clampDay("2026-12-01", "2026-07-19", "2026-08-17")).toBe("2026-08-17");
		expect(clampDay("2026-08-01", "2026-07-19", "2026-08-17")).toBe("2026-08-01");
	});
});
...
describe("rangeSpanAt", () => {
	...
	it("paints nothing without both ends, or outside them", () => {
		expect(rangeSpanAt("2026-07-20", week, 1, undefined, "2026-08-17")).toBeNull();
		expect(rangeSpanAt("2026-07-20", week, 1, "2026-07-19", undefined)).toBeNull();
		expect(rangeSpanAt("2026-07-18", week, 1, "2026-07-19", "2026-08-17")).toBeNull();
		expect(rangeSpanAt("2026-08-18", week, 1, "2026-07-19", "2026-08-17")).toBeNull();
	});

	it("paints nothing when the range is inverted", () => {
		expect(rangeSpanAt("2026-07-20", week, 1, "2026-08-17", "2026-07-19")).toBeNull();
	});

	it("opens at the range start and closes at its end", () => {
		expect(rangeSpanAt("2026-07-19", week, 0, "2026-07-19", "2026-07-22")).toEqual({
			open: true,
			close: false,
		});
		expect(rangeSpanAt("2026-07-22", week, 3, "2026-07-19", "2026-07-22")).toEqual({
			open: false,
			close: true,
		});
	});
	...
	it("caps both ends of a single-day range", () => {
		expect(rangeSpanAt("2026-07-21", week, 2, "2026-07-21", "2026-07-21")).toEqual({
			open: true,
			close: true,
		});
	});
