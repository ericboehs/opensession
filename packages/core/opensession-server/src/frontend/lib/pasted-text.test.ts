...
const attachment = (text: string, id = text): PastedTextAttachment => ({ id, text });

describe("pasted text attachments", () => {
  test("collapse starts at 2500 characters", () => {
    expect(shouldCollapsePastedText("x".repeat(PASTED_TEXT_THRESHOLD - 1))).toBe(false);
    expect(shouldCollapsePastedText("x".repeat(PASTED_TEXT_THRESHOLD))).toBe(true);
  });
  ...
  test("keeps pasted context ahead of the visible instruction", () => {
    expect(
      composePastedText("Summarize this", [attachment("First block"), attachment("Second block")]),
    ).toBe("First block\n\nSecond block\n\nSummarize this");
    expect(composePastedText("", [attachment("Only block")])).toBe("Only block");
