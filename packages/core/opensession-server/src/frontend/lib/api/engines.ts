	...
	const engines = Array.isArray(raw.engines)
		? (raw.engines as unknown[]).flatMap((entry) => {
				...
				return typeof e.id === "string" && typeof e.label === "string"
					? [{ id: e.id as EngineId, label: e.label, available: e.available !== false }]
					: [];
			})
		: [];
	const modelEngines: Record<string, EngineId> = {};
	if (raw.modelEngines && typeof raw.modelEngines === "object" && !Array.isArray(raw.modelEngines)) {
		for (const [model, engine] of Object.entries(raw.modelEngines as Record<string, unknown>)) {
...
export async function setModelEngineDefault(
	...
	const body = await request<{ modelEngines?: Record<string, EngineId> }>(
		...
		{ method: "PUT", body: { model, engine }, label: "Failed to save the default engine" },
	);
