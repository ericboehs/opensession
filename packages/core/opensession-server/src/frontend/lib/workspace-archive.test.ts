...
const session = (over: Partial<UnifiedSession> & { id: string }): UnifiedSession =>
	({
...
describe("workspaceArchivedSessions", () => {
	...
	test("adopts a duplicate workspace's sessions through the shared worktree", () => {
		...
		const rows = workspaceArchivedSessions({
			sessions: [],
			fetched: [
				session({ id: "sibling", workspaceId: "ws-ghpr", archived: true, worktreeDir: WT }),
			],
			workspaceId: "ws-mine",
			worktreeDir: WT,
		});
	...
	test("a shared checkout does not group sessions", () => {
		...
		expect(
			workspaceArchivedSessions({
				sessions: [session({ id: "other", archived: true, worktreeDir: shared })],
				workspaceId: "ws-mine",
				worktreeDir: shared,
			}),
		).toEqual([]);
	...
	test("the open session keeps its live tab, so it never lists", () => {
		expect(
			workspaceArchivedSessions({
				sessions: [session({ id: "open", workspaceId: "ws-1", archived: true })],
				workspaceId: "ws-1",
				excludeId: "open",
			}),
		).toEqual([]);
	...
	test("keeps automated and worker sessions", () => {
		const rows = workspaceArchivedSessions({
			sessions: [
				session({ id: "worker", workspaceId: "ws-1", archived: true, parentSessionId: "p" }),
				session({ id: "review", workspaceId: "ws-1", archived: true, automation: "github" }),
			],
			workspaceId: "ws-1",
		});
	...
	test("a session archived here lists before the fetch catches up", () => {
		const rows = workspaceArchivedSessions({
			sessions: [session({ id: "justClosed", workspaceId: "ws-1", archived: true })],
			fetched: [],
			workspaceId: "ws-1",
		});
	...
	test("the in-memory row wins over the slim fetched copy", () => {
		const rows = workspaceArchivedSessions({
			sessions: [
				session({ id: "a", workspaceId: "ws-1", archived: true, title: "Renamed" }),
			],
			fetched: [
				session({ id: "a", workspaceId: "ws-1", archived: true, title: "Stale", slim: true }),
			],
			workspaceId: "ws-1",
		});
