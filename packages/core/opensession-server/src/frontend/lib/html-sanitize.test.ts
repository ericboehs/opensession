...
describe("sanitizeHtmlFragment allowed markup", () => {
	...
	it("sends an external link to a new tab, leaves an anchor in place", () => {
		expect(sanitizeHtmlFragment('<a href="https://example.com">x</a>')).toContain(
...
describe("sanitizeHtmlFragment untrusted markup", () => {
	it("escapes tags that are not on the allowlist", () => {
		...
		expect(sanitizeHtmlFragment('<iframe src="https://evil.test"></iframe>')).toBe(
	...
	it("drops every attribute that is not allowed for the tag", () => {
		expect(sanitizeHtmlFragment('<img src="x.png" onerror="alert(1)">')).not.toContain(
			"onerror",
		);
		...
		expect(
			sanitizeHtmlFragment('<a href="https://ok.test" onclick="alert(1)">x</a>'),
