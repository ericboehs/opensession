...
test("the optimistic phone header names the model before its catalog loads", async () => {
	...
	const create = await Bun.file(new URL("../NewSession.tsx", import.meta.url)).text();

	expect(viewer).toContain("(hasWorkspace || effectiveModel || models.length > 0)");
	expect(viewer).toContain("{effectiveModel && (");
	expect(create).toContain("const optimisticModel = model || defaultModel;");
	expect(create).toContain("...(optimisticModel ? { model: optimisticModel } : {})");
});
