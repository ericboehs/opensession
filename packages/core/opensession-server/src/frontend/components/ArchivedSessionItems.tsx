			...
			{sessions.map((s) => (
				<Menu.Item key={s.id} onClick={() => onSelect(s)}>
					{(isAutomationSession(s) || !!s.parentSessionId) && (
						<IconRobot size={14} {...stylex.props(sx.shrink0, sx.textFaint)} aria-label="Agent run" />
					)}
					...
					<button
						type="button"
						className="hover:text-fg" {...stylex.props(sx.flex, sx.shrink0, sx.cursorPointer, sx.itemsCenter, sx.roundedControl, sx.border0, sx.bgTransparent, sx.p05, sx.textDim)}
