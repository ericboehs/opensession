  ...
  return (
    <SettingsPanel>
      ...
      {!body ? <SettingCardSkeleton rows={3} label="Loading failed deliveries" /> : entries.length === 0 ? (
        ...
        <SettingCard>
          {entries.map((entry) => (
            <div key={`${entry.timerId ? "timer" : "outbox"}:${entry.sessionId}:${entry.timerId || entry.id}`} {...mergeStylexProps("last:border-b-0", sx.phoneItemsStart, sx.phoneFlexCol, sx.flex, sx.minH14, sx.itemsCenter, sx.gap3, sx.borderB, sx.borderLine, sx.px3, sx.py2)}>
              ...
              <div {...mergeStylexProps("", sx.phoneWFull, sx.flex, sx.shrink0, sx.gap2)}>
                <button {...mergeStylexProps("", sx.activeScale096, sx.minH10, sx.roundedMd, sx.bgButton, sx.px3, sx.textSm, sx.textFg, sx.transitionTransform)} onClick={() => void change(entry, "retry").catch((cause) => setError(String(cause)))}>Retry</button>
                <button {...mergeStylexProps("text-danger", sx.hoverBgHover, sx.activeScale096, sx.transitionBackgroundColorScale, sx.minH10, sx.roundedMd, sx.px3, sx.textSm)} onClick={() => void change(entry, "discard").catch((cause) => setError(String(cause)))}>Discard</button>
              </div>
            </div>
          ))}
        </SettingCard>
      ...
      {body?.nextOffset !== undefined && (
        <button {...mergeStylexProps("", sx.activeScale096, sx.minH10, sx.roundedMd, sx.bgButton, sx.px3, sx.textSm, sx.textFg, sx.transitionTransform)} onClick={() => void loadMore().catch((cause) => setError(String(cause)))}>Load more</button>
      )}
      ...
      {pendingSends.length === 0 ? (
        ...
        <SettingCard>
          {pendingSends.map(({ command, outbox }) => (
            <div key={command.requestId} {...mergeStylexProps("last:border-b-0", sx.phoneItemsStart, sx.phoneFlexCol, sx.flex, sx.minH14, sx.itemsCenter, sx.gap3, sx.borderB, sx.borderLine, sx.px3, sx.py2)}>
              <div {...stylex.props(sx.minW0, sx.flex1)}>
              ...
              <div {...mergeStylexProps("", sx.phoneWFull, sx.flex, sx.shrink0, sx.gap2)}>
                <button {...mergeStylexProps("", sx.activeScale096, sx.minH10, sx.roundedMd, sx.bgButton, sx.px3, sx.textSm, sx.textFg, sx.transitionTransform)} onClick={() => window.dispatchEvent(new Event("opensession-command-outbox-retry"))}>Retry</button>
                <button {...mergeStylexProps("text-danger", sx.hoverBgHover, sx.activeScale096, sx.transitionBackgroundColorScale, sx.minH10, sx.roundedMd, sx.px3, sx.textSm)} onClick={() => {
