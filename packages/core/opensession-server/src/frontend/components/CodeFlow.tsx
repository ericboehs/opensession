...
export function CodeFlow({
	...
	if (loading && !data) return <LoadingState className="min-h-48">Mapping code flow…</LoadingState>;
	if (error && !data) {
		return (
			<InlineAlert className="m-4" onRetry={onRetry}>{error}</InlineAlert>
		);
	}
	...
	return (
		<section className="mx-auto w-full max-w-[1100px] px-3 py-4 phone:px-2" aria-labelledby={titleId}>
					...
					<article key={file} className="overflow-hidden rounded-xl bg-panel">
						<header className="flex min-h-10 items-center border-b border-divider bg-raised px-3 phone:px-2">
							{file !== "Project structure" && onOpenLocation ? (
								...
								<span className="font-mono text-xs font-semibold text-fg">{file}</span>
							)}
