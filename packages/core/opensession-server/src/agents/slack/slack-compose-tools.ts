	...
	return createSdkMcpServer({
		...
		tools: [
			tool(
				...
				{
					message: z.string().max(500).optional().describe("Draft Slack message, editable before sending."),
					channel: z.string().optional().describe("Optional configured channel name or id to preselect."),
					images: z.array(z.string()).max(10).optional().describe("Optional absolute image paths under /tmp or the service home to preview in the composer."),
				},
				async (args: { message?: string; channel?: string; images?: string[] }, extra: any) => {
					try {
