...
import { announcedSessionFile, bksIdFor, type AnnouncedRun } from "./run";
...
test("a first announce fills in everything the sessions list reads", () => {
	...
	const file = announcedSessionFile({} as NativeSessionFile, id, run, "ws-1", "app");
...
test("a later round keeps the engine session and the original createdAt", () => {
	...
	const file = announcedSessionFile(
		existing,
		id,
		{ ...run, prNumber: 4243, kind: "autofix", title: "Auto-fix · PR #4243 New" },
		null,
		"app",
	);
